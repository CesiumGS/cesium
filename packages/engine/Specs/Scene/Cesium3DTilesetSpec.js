...
describe("Scene/Cesium3DTileset", function () {
  ...
  const texturedUrl = "Data/Cesium3DTiles/Batched/BatchedTextured/tileset.json";
  ...
  it("fromUrl rejects with undefined resource", function () {
    return expectAsync(Cesium3DTileset.fromUrl()).toBeRejectedWithError(
      DeveloperError,
      "url is required, actual value was undefined"
    );
  });

  it("fromUrl rejects if url is not valid", async function () {
    const path = "invalid.json";
    return expectAsync(Cesium3DTileset.fromUrl(path)).toBeRejectedWith(
      RequestErrorEvent,
      "Request has failed. Status Code: 404"
    );
  });

  it("fromUrl returns Cesium3DTileset instance from url", async function () {
    const url = "Data/Cesium3DTiles/Tilesets/TilesetOfTilesets/tileset.json";
    const result = await Cesium3DTileset.fromUrl(url);
    expect(result).toBeInstanceOf(Cesium3DTileset);
    expect(result.resource).toBeInstanceOf(Resource);
    expect(result.resource.url).toEqual(url);
  });

  it("fromUrl returns Cesium3DTileset instance from promise to url", async function () {
    const url = "Data/Cesium3DTiles/Tilesets/TilesetOfTilesets/tileset.json";
    const result = await Cesium3DTileset.fromUrl(Promise.resolve(url));
    expect(result).toBeInstanceOf(Cesium3DTileset);
    expect(result.resource).toBeInstanceOf(Resource);
    expect(result.resource.url).toEqual(url);
  });

  it("fromUrl returns Cesium3DTileset instance from promise to resource", async function () {
    const resource = new Resource({
      url: "Data/Cesium3DTiles/Tilesets/TilesetOfTilesets/tileset.json",
    });
    const result = await Cesium3DTileset.fromUrl(Promise.resolve(resource));
    expect(result).toBeInstanceOf(Cesium3DTileset);
    expect(result.resource).toBeInstanceOf(Resource);
    expect(result.resource.url).toEqual(resource.url);
  });

  it("fromUrl returns Cesium3DTileset instance from file resource", async function () {
    const resource = new Resource({
      url: "Data/Cesium3DTiles/Tilesets/TilesetOfTilesets/tileset.json",
    });
    const result = await Cesium3DTileset.fromUrl(resource);
    expect(result).toBeInstanceOf(Cesium3DTileset);
    expect(result.resource).toBeInstanceOf(Resource);
    expect(result.resource.url).toEqual(resource.url);
  });

  it("fromUrl rejects with invalid tileset version", async function () {
    const tilesetJson = {
      asset: {
        version: "1.2",
      },
    ...
    return expectAsync(Cesium3DTileset.fromUrl(uri)).toBeRejectedWithError(
      RuntimeError,
      "The tileset must be 3D Tiles version 0.0, 1.0, 1.1, or 2.0"
    );
  });

  it("fromUrl rejects with unsupported extension", async function () {
    const tilesetJson = {
      asset: {
        version: "1.0",
      },
  ...
  it("fromUrl sets resource.url to the given tileset JSON url with query string", async function () {
    const url = "Data/Cesium3DTiles/Tilesets/TilesetOfTilesets/tileset.json";
    const params = "?param1=1&param2=2";
    const tileset = await Cesium3DTileset.fromUrl(url + params);
    expect(tileset.resource.url).toEqual(url + params);
  });

  it("fromUrl returns Cesium3DTileset loads tileset JSON file", async function () {
  ...
  it("gets root child throws if not loaded", async function () {
    const tileset = await Cesium3DTileset.fromUrl(tilesetUrl);
  ...
  it("gets root child when loaded", async function () {
    const tileset = await Cesium3DTileset.fromUrl(tilesetUrl);
    await tileset.load();

    expect(tileset.root).toBeInstanceOf(Cesium3DTile);
  });

  it("hasExtension returns true if the tileset JSON file uses the specified extension", async function () {
    const tileset = await Cesium3DTileset.fromUrl(withBatchTableHierarchyUrl);
  ...
  it("passes version in query string to tiles", async function () {
    ...
    expect(tileset.root._contentResource.url).toEqual(
      getAbsoluteUri(tilesetUrl.replace("tileset.json", "parent.b3dm?v=1.2.3"))
    );
  });

  it("passes version in query string to all external resources", async function () {
    const queryParams = "?a=1&b=boy";
    const queryParamsWithVersion = "?v=1.2.3&a=1&b=boy";
    const tileset = await Cesium3DTileset.fromUrl(tilesetUrl + queryParams);
    await tileset.load();

    let url = tileset.root._contentResource.url;
    ...
    const length = tileset.root.children.length;
    for (let i = 0; i < length; ++i) {
      if (defined(tileset.root.children[i].content)) {
        url = tileset.root.children[i]._contentResource.url;
        const query = url.slice(url.indexOf("?"));
        if (url.indexOf("tileset.json") >= 0) {
          // The initial tileset.json does not have a tileset version parameter
          expect(query).toBe(queryParams);
  ...
  describe(
    ...
    function () {
      ...
      // TODO: Does this even belong in this test suite??
      xit("requests tile with invalid magic", async function () {
        ...
        return tileset.readyPromise.then(function (tileset) {
          ...
          return root.contentReadyPromise
            ...
            .catch(function (error) {
              ...
              expect(root._contentState).toEqual(
      ...
      it("handles failed tile requests", async function () {
        ...
        const root = tileset.root;
        root._contentResource.url = "fake.b3dm";
        scene.renderForSpecs(); // Request root content

        await expectAsync(root.contentReadyPromise).toBeRejected();
      ...
      it("verify statistics", async function () {
        ...
        return Cesium3DTilesTester.waitForReady(scene, tileset).then(
          function () {
      ...
      it("destroys before tile finishes loading", async function () {
        ...
        const content = await root.contentReadyPromise;
        expect(content).toBeUndefined();
      ...
      describe("most detailed height queries", function () {
        ...
        it("tileset uses additive refinement", function () {
          ...
          const originalLoadJson = Cesium3DTileset.loadJson;
          spyOn(Cesium3DTileset, "loadJson").and.callFake(function (
            tilesetUrl
          ) {
            return originalLoadJson(tilesetUrl).then(function (tilesetJson) {
      ...
      describe("multiple contents", function () {
        ...
        it("request statistics are updated for partial success", async function () {
          ...
          await tileset.root.contentReadyToProcessPromise;
          expect(statistics.numberOfAttemptedRequests).toBe(0);
          expect(statistics.numberOfPendingRequests).toBe(0);
      ...
      describe("3DTILES_multiple_contents", function () {
        ...
        it("raises tileFailed for external tileset inside multiple contents (legacy)", async function () {
          const originalLoadJson = Cesium3DTileset.loadJson;
          spyOn(Cesium3DTileset, "loadJson").and.callFake(function (
            tilesetUrl
          ) {
      ...
      describe("metadata", function () {
        ...
        it("gracefully handles external tileset with content metadata but no root schema", function () {
          ...
          return Cesium3DTilesTester.loadTileset(
            ...
            expect(findContentMetadata._oneTimeWarning).toHaveBeenCalledTimes(
              5
            );
