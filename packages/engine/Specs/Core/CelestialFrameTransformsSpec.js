...
describe("Core/CelestialFrameTransforms", function () {
  it("computeTemeToPseudoFixedMatrix works before noon", function () {
    ...
    const t = CelestialFrameTransforms.computeTemeToPseudoFixedMatrix(time);
    ...
    const u = CelestialFrameTransforms.computeTemeToPseudoFixedMatrix(time);
  ...
  it("computeTemeToPseudoFixedMatrix works with a result parameter", function () {
    ...
    const t = CelestialFrameTransforms.computeTemeToPseudoFixedMatrix(
      time,
      resultT,
    );
    ...
    const u = CelestialFrameTransforms.computeTemeToPseudoFixedMatrix(
      time,
      resultU,
    );
  ...
  describe("computeIcrfToMoonFixedMatrix", function () {
    it("throws if the date parameter is not specified", function () {
      expect(function () {
        CelestialFrameTransforms.computeIcrfToMoonFixedMatrix(undefined);
      }).toThrowDeveloperError();

      expect(function () {
        CelestialFrameTransforms.computeFixedToIcrfMatrix(undefined);
      }).toThrowDeveloperError();
    });
    ...
    it("works", async function () {
      ...
      const resultT = new Matrix3();
      const t = CelestialFrameTransforms.computeIcrfToMoonFixedMatrix(
        time,
        resultT,
      );
      expect(t).toBe(resultT);
      ...
      const resultU = new Matrix3();
      const u = CelestialFrameTransforms.computeIcrfToMoonFixedMatrix(
        time,
        resultU,
      );
      expect(u).toBe(resultU);
  ...
  describe("computeIcrfToFixedMatrix", function () {
    async function preloadTransformationData(start, stop, eopUrl) {
      ...
      await CelestialFrameTransforms.preloadIcrfFixed(preloadInterval);
    }

    it("throws if the date parameter is not specified", function () {
      expect(function () {
        CelestialFrameTransforms.computeIcrfToFixedMatrix(undefined);
      }).toThrowDeveloperError();
    ...
    it("works with data from STK Components", async function () {
      ...
      for (let i = 0; i < componentsData.length; ++i) {
        ...
        const resultT = new Matrix3();
        const t = CelestialFrameTransforms.computeIcrfToFixedMatrix(
          time,
          resultT,
        );
        expect(t).toBe(resultT);
    ...
    it("works with hard-coded data", async function () {
      ...
      time = JulianDate.addHours(time, 23.93447, new JulianDate()); // add one sidereal day
      const resultU = new Matrix3();
      const u = CelestialFrameTransforms.computeIcrfToFixedMatrix(
        time,
        resultU,
      );
      expect(u).toBe(resultU);
    ...
    it("throws a RuntimeError when asked to compute with invalid EOP data", async function () {
      ...
      await expectAsync(
        (async function () {
          ...
          return CelestialFrameTransforms.computeIcrfToFixedMatrix(time);
        })(),
    ...
    it("returns undefined before XYS data is loaded.", function () {
      ...
      const time = new JulianDate(2455745, 43200);
      expect(
        CelestialFrameTransforms.computeIcrfToFixedMatrix(time),
      ).toBeUndefined();
    });
  });
  ...
  it("computeTemeToPseudoFixedMatrix throws without a date", function () {
    expect(function () {
      CelestialFrameTransforms.computeTemeToPseudoFixedMatrix(undefined);
    }).toThrowDeveloperError();
  });
});
