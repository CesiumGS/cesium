 ...
 * Instead, we use v_positionEC, and let {@link czm_eyeToCartographicDelta} compute a geodetic delta (long, lat) between the eye and vertex. This delta is a smaller quantity (and gets smaller and more precise as we zoom in).
 * Then, we can combine this delta with the eye's own UV coordinate within the model's rectangle to obtain the vertex's uv coordinate.
 * Note that both terms are relative to the eye of the pass being rendered, which is not the scene camera in passes such as shadow casting.
 *
...
void modelClippingPolygonsStage()
{
    ...
    // Express the vertex's position within the model's rectangle as a normalized uv coordinate, by adding the eye's uv and scaling by the rectangle's size.
    // Since the eye's UV is computed on the CPU in double precision, and gets more precise as one zooms in, this ensures good precision on the vertex's uv.
