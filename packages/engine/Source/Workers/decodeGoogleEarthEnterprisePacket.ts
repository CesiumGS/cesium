...
function decodeGoogleEarthEnterprisePacket(parameters: any, transferableObjects: any) {
...
function processMetadata(buffer: any, totalSize: any, quadKey: any) {
  ...
  function populateTiles(parentKey: any, parent: any, level: any) {
    let isLeaf = false;
    if (level === 4) {
...
// which are ignored.
function processTerrain(buffer: any, totalSize: any, transferableObjects: any) {
  const dv = new DataView(buffer);
...
function uncompressPacket(data: any) {
