 ...
 */
function incrementallyBuildTerrainPicker(parameters: any, transferableObjects: any) {
  // Rehydrate worker inputs
  const aabbs = new Float64Array(parameters.aabbs);
  const nodeAABBs = Array.from({ length: 4 }, (_: any, i: any) => {
 ...
 */
function createAABBFromTriangle(inverseTransform: any, trianglePoints: any) {
