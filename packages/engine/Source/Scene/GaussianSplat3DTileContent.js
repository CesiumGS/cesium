...
import GaussianSplatData from "./GaussianSplatData.js";

export const GaussianSplatTextureGeneratorState = {
  UNINITIALIZED: 0,
  PENDING: 1,
  READY: 2,
  ERROR: 3,
...
GaussianSplat3DTileContent.prototype.update = function (primitive, frameState) {
  ...
  if (this._ready) {
    ...
    if (
      ...
      if (this._sphericalHarmonicsDegree > 0) {
        const packedData = GaussianSplatData.packSphericalHarmonicsData(this);
        this.sh1TextureData = GaussianSplatData.makeSh1TextureData(packedData);

        if (this._sphericalHarmonicsDegree > 1) {
          this.sh2TextureData =
            GaussianSplatData.makeSh2TextureData(packedData);
        }

        if (this._sphericalHarmonicsDegree > 2) {
          this.sh3TextureData =
            GaussianSplatData.makeSh3TextureData(packedData);
        }
      }
  ...
  if (this._resourcesLoaded) {
    ...
    const { l, n } = GaussianSplatData.degreeAndCoefFromAttributes(
