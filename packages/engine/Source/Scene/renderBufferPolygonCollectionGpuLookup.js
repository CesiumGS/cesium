...
const polygonMaskFragmentShader = `
in vec2 v_uv;

uniform highp sampler2D u_maskTexture;
 ...
 * @returns {BufferPolygon|undefined}
 */
function getFirstVisiblePolygon(collection, result) {
...
function createLookupTextures(context, lookup) {
  ...
  if (defined(lookup.maskTexels)) {
    return {
      // @ts-expect-error Private in TS declarations, used internally by renderer code.
      maskTexture: Texture.create({
        context: context,
        pixelFormat: PixelFormat.RED,
        pixelDatatype: PixelDatatype.UNSIGNED_BYTE,
        source: {
          width: lookup.maskTextureWidth,
          height: lookup.maskTextureHeight,
          arrayBufferView: lookup.maskTexels,
        },
        sampler: sampler,
        flipY: false,
      }),
...
function destroyRenderContext() {
  ...
  if (defined(this.maskTexture) && !this.maskTexture.isDestroyed()) {
 ...
 * @returns {*}
 */
export default function renderBufferPolygonCollectionGpuLookup(
  collection,
  frameState,
  renderContext,
) {
  ...
  if (needsSharedRebuild || needsCollectionRebuild) {
    ...
    const lookup = defined(sharedLookup)
      ? sharedLookup
      : buildPolygonCollectionGpuLookup(collection);
    if (!defined(lookup)) {
      return undefined;
    }
    ...
    renderContext.maskTexture = textures.maskTexture;
  ...
  if (!defined(renderContext.shaderProgram)) {
    renderContext.shaderProgram = ShaderProgram.fromCache({
      ...
      fragmentShaderSource: new ShaderSource({
        sources: [
          defined(renderContext.lookup?.maskTexels)
            ? polygonMaskFragmentShader
            : BufferPolygonCollectionGpuLookupFS,
        ],
      }),
  ...
  if (
    ...
    renderContext.command = new DrawCommand({
      ...
      uniformMap: {
        u_maskTexture: function () {
          return renderContext.maskTexture;
        },
