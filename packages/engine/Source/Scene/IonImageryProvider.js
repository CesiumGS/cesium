...
const ImageryProviderAsyncMapping = {
  ...
  GOOGLE_2D_MAPS: (url, endpointOptions, attributions) => {
    const createSessionCallback = (ionResource, newEndpoint) => {
      // Create a new session using updated session token and Google API key generated by ion
      ionResource.setQueryParameters({
        session: newEndpoint.options.session,
        key: newEndpoint.options.key,
      });
    };
    const ionResource = IonResource.fromExternalEndpoint({
      url,
      attributions,
      endpointOptions,
  ...
  AZURE_MAPS: (url, endpointOptions, attributions) => {
    const createSessionCallback = (ionResource, newEndpoint) => {
      // Create a new session using a new Azure subscription key generated by ion
      ionResource.setQueryParameters({
...
IonImageryProvider.fromAssetId = async function (assetId, options) {
  ...
  if (!defined(externalType)) {
    ...
    delete options.url;
    imageryProvider = await factory(url, options, endpoint.attributions);
  }
