...
export default function geojsonToArray(features, bbox, useTest = false) {
...
export function geojsonToArrayInGrid(
  features,
  bbox,
    ...
    if (geometry.type === "LineString") {
      ...
      for (let j = 0; j < lineCoords.length - 1; j++) {
        ...
        const cellX1 = Math.min(
          Math.floor((x1 - bbox[0]) / cellWidth),
          gridSizeX - 1,
        );
        const cellY1 = Math.min(
          Math.floor((y1 - bbox[1]) / cellHeight),
          gridSizeY - 1,
        );
        const cellX2 = Math.min(
          Math.floor((x2 - bbox[0]) / cellWidth),
          gridSizeX - 1,
        );
        const cellY2 = Math.min(
          Math.floor((y2 - bbox[1]) / cellHeight),
          gridSizeY - 1,
        );
