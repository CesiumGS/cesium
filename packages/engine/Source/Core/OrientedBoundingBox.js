...
class OrientedBoundingBox {
  ...
  static fromRectangle(
    ...
    maxX = Cartesian3.dot(
      Plane.projectPointOntoPlane(
        plane,
        horizonCartesian,
        // @ts-expect-error Requires conditional or generic return types in tsd-jsdoc.
   ...
   */
  static intersectPlane(box, plane) {
    //>>includeStart('debug', pragmas.debug);
    if (!defined(box)) {
   ...
   * });
   */
  static distanceSquaredTo(box, cartesian) {
   ...
   */
  static computePlaneDistances(box, position, direction, result) {
    //>>includeStart('debug', pragmas.debug);
    if (!defined(box)) {
      throw new DeveloperError("box is required.");
    }
   ...
   */
  static computeCorners(box, result) {
    //>>includeStart('debug', pragmas.debug);
   ...
   */
  static computeTransformation(box, result) {
    //>>includeStart('debug', pragmas.debug);
    Check.typeOf.object("box", box);
    //>>includeEnd('debug');
   ...
   */
  static isOccluded(box, occluder) {
    //>>includeStart('debug', pragmas.debug);
    if (!defined(box)) {
      throw new DeveloperError("box is required.");
    }
...
/**
 * @param {Cartesian3} planeOrigin
 * @param {Cartesian3} planeXAxis
 * @param {Cartesian3} planeYAxis
 * @param {Cartesian3} planeZAxis
