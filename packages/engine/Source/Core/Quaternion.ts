...
/**
 * Interface for HeadingPitchRoll-like objects used in Quaternion operations
 */
...
/**
 * Interface for Matrix3-like objects used in Quaternion operations
 */
...
/**
 * Interface for Cartesian3-like objects used in Quaternion operations
 */
...
// Module-level scratch variables for performance
...
const v = FeatureDetection.supportsTypedArrays() ? new Float32Array(8) : [] as number[];
const bT = FeatureDetection.supportsTypedArrays() ? new Float32Array(8) : [] as number[];
const bD = FeatureDetection.supportsTypedArrays() ? new Float32Array(8) : [] as number[];
...
class Quaternion {
   ...
   */
  static readonly packedInterpolationLength: number = 3;

  /**
   * An immutable Quaternion instance initialized to (0.0, 0.0, 0.0, 0.0).
   */
  static readonly ZERO: Readonly<Quaternion> = Object.freeze(
   ...
   */
  static readonly IDENTITY: Readonly<Quaternion> = Object.freeze(
  ...
  static fromAxisAngle(
    ...
    fromAxisAngleScratch = Cartesian3.normalize(
      axis as Cartesian3,
   ...
   */
  static fromRotationMatrix(matrix: Matrix3Like, result?: Quaternion): Quaternion {
    //>>includeStart('debug', pragmas.debug);
  ...
  static fromHeadingPitchRoll(
    headingPitchRoll: HeadingPitchRollLike,
    result?: Quaternion
  ): Quaternion {
    //>>includeStart('debug', pragmas.debug);
    ...
    let scratchRollQuaternion: Quaternion;
    let scratchPitchQuaternion: Quaternion;
    let scratchHeadingQuaternion: Quaternion;
   ...
   * @param startingIndex - The index of the first element to be converted. Default is 0.
   * @param lastIndex - The index of the last element to be converted. Default is packedArray.length.
   * @param result - The object into which to store the result.
  ...
  static convertPackedArrayForInterpolation(
    packedArray: number[],
    startingIndex: number = 0,
    lastIndex: number = packedArray.length,
    result: number[] = []
  ): number[] {
    Quaternion.unpack(
      packedArray,
   ...
   * @param firstIndex - The firstIndex used to convert the array. Default is 0.
   * @param lastIndex - The lastIndex used to convert the array. Default is packedArray.length.
   * @param result - The object into which to store the result.
   * @returns The modified result parameter or a new Quaternion instance if one was not provided.
   */
  static unpackInterpolationResult(
    ...
    sourceArray: number[],
    firstIndex: number = 0,
    lastIndex: number = sourceArray.length,
    result?: Quaternion
  ): Quaternion {
    if (!defined(result)) {
      result = new Quaternion();
   ...
   *
   * @param quaternion - The quaternion to compute magnitude squared for.
   ...
   *
   * @param quaternion - The quaternion to compute magnitude for.
  ...
  static log(quaternion: Quaternion, result: Cartesian3): Cartesian3 {
    ...
    return Cartesian3.multiplyByScalar(
      quaternion as unknown as Cartesian3,
      thetaOverSinTheta,
      result
    );
  }
