...
define([
    ...
    function updateTransform_NorthUp(that, camera, objectChanged, positionProperty, time, ellipsoid) {
        var cartesian = positionProperty.getValue(time, that._lastCartesian);
        if (defined(cartesian)) {
            // X along the nadir
            var xBasis = updateTransformCartesian3Scratch2;
            Cartesian3.normalize(cartesian, xBasis);
            Cartesian3.negate(xBasis, xBasis);

            // Z is North
            var zBasis = Cartesian3.clone(Cartesian3.UNIT_Z, updateTransformCartesian3Scratch3);

            // Y is along the cross of z and x (right handed basis / in the direction of motion)
            var yBasis = Cartesian3.cross(zBasis, xBasis, updateTransformCartesian3Scratch1);
    ...
    DynamicObjectView.prototype.update = function(time) {
        ...
        if (scene.mode !== SceneMode.MORPHING) {
            if (this.rotationMode === StoredViewCameraRotationMode.NORTH_UP) {
                updateTransform_NorthUp(this, scene.camera, objectChanged, positionProperty, time, ellipsoid);
            } else {
