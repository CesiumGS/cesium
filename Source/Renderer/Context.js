...
define([
     ...
     * ////////////////////////////////////////////////////////////////////////////////
     *
     * // Example 2.  When the caller destroys the vertex array, it also destroys the
    ...
    Context.prototype.createVertexArrayFromGeometry = function(creationArguments) {
        ...
        for (name in attributes) {
            if (attributes.hasOwnProperty(name)) {
                ...
                if (typeof attribute.interleavedValues !== 'undefined') {
                    var values = attribute.interleavedValues;
                    var length = interleavedBuffers.length;
                    for (var i = 0; i < length; ++i) {
                        var interleavedBuffer = interleavedBuffers[i];
                        if (values === interleavedBuffer.values) {
                            vertexBuffer = interleavedBuffer.vertexBuffer;
                            break;
                        }
                    }

                    if (typeof vertexBuffer === 'undefined') {
                        vertexBuffer = this.createVertexBuffer(values, bufferUsage);
                        interleavedBuffers.push({
                            values : values,
                            vertexBuffer : vertexBuffer
                        });
                    }
                } else if (typeof attribute.values !== 'undefined') {
