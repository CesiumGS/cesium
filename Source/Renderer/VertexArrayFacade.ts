define([
    ...
    'use strict';
        ...
        /**
             * @private
             */
        class VertexArrayFacade {
            constructor(context, attributes, sizeInVertices, instanced) {
            ...
            subCommit(offsetInVertices, lengthInVertices) {
                //>>includeStart('debug', pragmas.debug);
            ...
            static _verifyAttributes(attributes) {
                var attrs = [];
            ...
            static _vertexSizeInBytes(attributes) {
                var sizeInBytes = 0;
            ...
            static _createArrayViews(attributes, vertexSizeInBytes) {
                var views = [];
                var offsetInBytes = 0;
                var length = attributes.length;
                for (var i = 0; i < length; ++i) {
            ...
            static _appendWriters(writers, buffer) {
            ...
            static _appendAttributes(attributes, buffer, vertexBufferOffset, instanced) {
                var arrayViews = buffer.arrayViews;
                var length = arrayViews.length;
                for (var i = 0; i < length; ++i) {
