...
import Rectangle from './Rectangle.js';
    ...
    /**
     * Computes a point that can be used for horizon culling from a list of positions that might be underneath
     * the ellipsoid. If the minimum height of the positions is under the ellipsoid, the returned point is relative
     * to an ellipsoid that has been shrunk by the distance between the ellipsoid surface and the minimum height.
     * If the point is below the horizon, all of the positions are guaranteed to be below the horizon as well.
     * The returned point is expressed in the ellipsoid-scaled space and is suitable for use with
     * {@link EllipsoidalOccluder#isScaledSpacePointVisiblePossiblyUnderEllipsoid}.
     *
     * @param {Cartesian3} directionToPoint The direction that the computed point will lie along.
     *                     A reasonable direction to use is the direction from the center of the ellipsoid to
     *                     the center of the bounding sphere computed from the positions.  The direction need not
     ...
     *                       must be expressed in a reference frame centered at the ellipsoid and aligned with the
     *                       ellipsoid's axes.
     * @param {Number} [minimumHeight] The minimum height of all positions. If this value is undefined, all positions
     *                       are assumed to be above the ellipsoid.
     * @param {Cartesian3} [result] The instance on which to store the result instead of allocating a new instance.
     * @returns {Cartesian3} The computed horizon culling point, expressed in the ellipsoid-scaled space.
     */
    ...
    function computeHorizonCullingPointPossiblyUnderEllipsoid(ellipsoidalOccluder, minimumHeight, func) {
        ...
        if (defined(minimumHeight) && minimumHeight < 0.0 && ellipsoid.minimumRadius > -minimumHeight) {
            ...
            // The culling point is calculated with respect to the shrunked ellipsoid, so
            // temporarily change change the occluder's ellipsoid to the shrunken one.
            ellipsoidalOccluder._ellipsoid = ellipsoid;
            var result = func();
            ellipsoidalOccluder._ellipsoid = ellipsoidOrig;
            return result;
        }
