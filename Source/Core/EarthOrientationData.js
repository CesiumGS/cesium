    ...
    var EarthOrientationData = {
            ...
            computeOrientationParameters : function (dateTai){
                ...
                if(eop._ts_lastIndex < eop._eop.length){
                    if (eop._dates[eop._ts_lastIndex].lessThanOrEquals(dateTai) &&
                            (eop._ts_lastIndex === eop._dates.length - 1 ||
                                    eop._dates[eop._ts_lastIndex + 1].greaterThanOrEquals(dateTai))){
                        before = eop._ts_lastIndex;

                        if (before < eop._eop.length - 1 &&
                                eop._dates[before + 1].equals(dateTai)){
                            ++before;
                        }
                        after = before + 1;
                ...
                if (index >= 0) {
                    ...
                    if (index < eop._eop.length - 1 &&
                        eop._dates[index + 1].equals(dateTai))
                    {
                        ++index;
                    }
                    before = index;
            ...
            interpolateEopDates : function(desiredDate, before, after){
                ...
                var factor = beforeDate.getSecondsDifference(desiredDate) /
                ...
                return new EarthOrientationData.OrientationParameterData(
                        ...
                        ut1MinusUtc, beforeData.taiMinusUtc);
            },
            ...
            addOrientationParameterData : function(date, xVal, yVal, xPoleOffset, yPoleOffset, ut1MinusUtc, taiMinusUtc){
                var newData = new EarthOrientationData.OrientationParameterData(
                        xVal, yVal, xPoleOffset, yPoleOffset, ut1MinusUtc, taiMinusUtc);
