...
uniform float colorTemperatureInner;
uniform float colorTemperatureOuter;  
uniform float saturation;
uniform float postSaturation;
uniform float finalMultiplier;
uniform vec3 tintColorBottom;
uniform vec3 tintColorTop;

// Example custom uniform values...
// vignetteAmount = 0.65
// vignetteGamma = 4.0
// colorTemperatureInner = 3500.0
// colorTemperatureOuter = 12000.0
// saturation = 0.5
// postSaturation = 0.5
// finalMultiplier = 1.2
// tintColorBottom = { x : 1.1, y : 0.85, z : 0.95 }
// tintColorTop = { x : 1.1, y : 0.77, z : 0.55 }
...
vec3 tintColor()
{
        return mix(tintColorTop,tintColorBottom,1.0-v_textureCoordinates.y);
}
...
vec3 colorTemperatureFilter(vec3 rgb)
{
    // Work out an RGB value based on color temperature
    float ct = mix(colorTemperatureOuter, colorTemperatureInner, gammaVignette(1.0,3.0));
...
// Apply a lomo grade
vec3 lomoGradeFilter(vec3 rgb, float saturation, float finalMultiplier)
{
    ...
    //rgb = clamp(rgb, vec3(0.0), vec3(10.0));
        ...
        // Apply tint as a gamma    
    rgb = pow(rgb, vec3(1.0) / tintColor());
    ...
    rgb = colorTemperatureFilter(rgb);
...
void main(void)
{
    ...
    vec3 rgb = gammaCorrect(sRGB, 1.0 / 2.2);
        ...
        // Apply tint slightly as an offset
    rgb += (tintColor() * vec3(0.05));
    
    // Non-perfect lens
    rgb = gammaVignetteFilter(rgb, vignetteAmount, vignetteGamma);
       
    // Film processing / color grading
    rgb = lomoGradeFilter(rgb, saturation, finalMultiplier);

    // Approximate linear -> sRGB
    sRGB = gammaCorrect(rgb, 2.2);
    
           // Final desaturation
    sRGB = czm_saturation(sRGB, postSaturation);
