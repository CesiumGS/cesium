...
czm_ray getPickRay(vec2 screenPosition) {
    ...
    float y = (2.0 / height) * (height - screenPosition.y) - 1.0;

    float near = czm_currentFrustum.x;
    vec3 nearCenter = vec3(0.0, 0.0, -1.0) * near;
    vec3 xDir = vec3(1.0, 0.0, 0.0) * (x * czm_frustumPlanes.w);
    vec3 yDir = vec3(0.0, 1.0, 0.0) * (y * czm_frustumPlanes.x);
    vec3 direction = normalize(nearCenter + xDir + yDir);

    return czm_ray(vec3(0.0, 0.0, 0.0), direction);
}
