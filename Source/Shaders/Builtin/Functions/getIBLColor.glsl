vec3 czm_getIBLColor(vec3 toEye, vec3 positionWC, vec3 normal, vec3 diffuseColor, float roughness)
{
    ...
    return ambiantColor + diffuseIrradiance;// + (specularIrradiance * SRGBtoLINEAR3(specularColor * brdfLut.x + brdfLut.y));
}
