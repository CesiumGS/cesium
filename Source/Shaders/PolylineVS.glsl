...
void main() 
{
    ...
    bool usePrevDirection = texCoordExpandWidthAndShow.z < 0.0;
    ...
    if (u_morphTime == 1.0)
    {
        ...
        segmentMagnitude = usePrevDirection ? length(prev - p) : length(next - p);
        
        prevDir = vec4(normalize(prev.xyz - p.xyz), 0.0);
        nextDir = vec4(normalize(next.xyz - p.xyz), 0.0);
    }
    ...
    if (czm_equalsEpsilon(nextDir, vec4(0.0), czm_epsilon7))
    {
        nextDir = -prevDir;
    }
    ...
    vec4 nextEC = vec4(normalize((czm_modelViewRelativeToEye * nextDir).xyz), 0.0);
    vec4 prevEC = vec4(normalize((czm_modelViewRelativeToEye * prevDir).xyz), 0.0);
