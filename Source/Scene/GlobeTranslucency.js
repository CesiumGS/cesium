...
GlobeTranslucency.isSunVisibleThroughGlobe = function (
  globe,
  cameraUnderground
) {
  if (!defined(globe) || !globe.show) {
  ...
  var frontOpaque =
    getFrontFaceAlphaMode(translucencyMode) === TranslucencyMode.FRONT_OPAQUE;
  var backOpaque =
    getBackFaceAlphaMode(translucencyMode) === TranslucencyMode.BACK_OPAQUE;
...
function DerivedCommandPack(
  names,
  passes,
  pickOnly,
  getShaderProgramFunctions,
  getRenderStateFunctions,
  getUniformMapFunctions
) {
...
function getDerivedCommandPack(frameState) {
  ...
  if (!defined(globeTranslucency._derivedCommandPack)) {
    globeTranslucency._derivedCommandPack = new DerivedCommandPack(
      ...
      [
        getBackAndFrontFaceShaderProgram,
        getBackFaceShaderProgram,
        getFrontFaceShaderProgram,
        getTranslucentBackFaceShaderProgram,
        getTranslucentShaderProgram,
        getClearDepthTranslucentBackFaceShaderProgram,
        getClearDepthTranslucentFrontFaceShaderProgram,
        getPickShaderProgram,
        getPickShaderProgram,
      ],
      [
        getBackAndFrontFaceRenderState,
        getBackFaceRenderState,
        getFrontFaceRenderState,
        getTranslucentBackFaceRenderState,
        getTranslucentFrontFaceRenderState,
        getTranslucentBackFaceRenderState,
        getTranslucentFrontFaceRenderState,
        getPickBackFaceRenderState,
...
GlobeTranslucency.updateDerivedCommand = function (command, frameState) {
  ...
  if (!defined(derivedCommands) || command.dirty) {
    ...
    var getShaderProgramFunctions =
      derivedCommandsPack.getShaderProgramFunctions;
    ...
    for (i = 0; i < length; ++i) {
      ...
      commands[i].uniformMap = getDerivedUniformMap(
        frameState.globeTranslucency,
        command.uniformMap,
        uniformMaps[i],
        uniformMapChanged,
        getUniformMapFunctions[i]
      );
    ...
    if (
      ...
      derivedCommands.shaderProgramId = command.shaderProgram.id;
      for (i = 0; i < length; ++i) {
        commands[i].shaderProgram = getDerivedShaderProgram(
          frameState.context,
          command.shaderProgram,
          getShaderProgramFunctions[i],
          names[i]
        );
        commands[i].renderState = getDerivedRenderState(
          command.renderState,
          getRenderStateFunctions[i],
          renderStateCaches[i]
        );
      }
    } else {
...
function requireManualDepthTest(
  tileProvider,
  translucencyMode,
  frameState,
  scene2D
  ...
  return (
    ...
    !tileProvider.depthTestAgainstTerrain &&
    frameState.context.depthTexture &&
    !scene2D
  );
}

GlobeTranslucency.getNumberOfTextureUniforms = function (
  tileProvider,
  frameState
) {
  ...
  if (
    requireManualDepthTest(tileProvider, translucencyMode, frameState, scene2D)
  ) {
...
GlobeTranslucency.pushDerivedCommands = function (
  ...
  if (
    translucencyMode ===
    (TranslucencyMode.FRONT_INVISIBLE | TranslucencyMode.BACK_INVISIBLE)
  ) {
  ...
  var scene2D = frameState.mode === SceneMode.SCENE2D;
  var clearGlobeDepth = requireManualDepthTest(
    tileProvider,
    translucencyMode,
    frameState,
    scene2D
  ...
  var translucentFrontFaceCommand = picking
  ...
  var translucentBackFaceCommand = picking
    ? derivedCommands.pickBackFaceCommand
    : clearGlobeDepth
...
GlobeTranslucency.prototype.updateAndClear = function (
  hdr,
  viewport,
  context,
  passState
