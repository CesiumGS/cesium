define([
    ...
    // array[subtreeLevel] will give offset to the start of that 0Indexed level
    ...
    Cesium3DTilesetImplicit.prototype.getSubtreeInfoFromTreeKey = function(x, y, z, level) {
        ...
        var subtreeRootKey = new Cartesian4(
            x >> levelWithinSubtree,
            y >> levelWithinSubtree,
            z >> levelWithinSubtree,
            subtreeRootLevel
        );
        var shiftX = (subtreeRootKey.x << levelWithinSubtree);
        var shiftY = (subtreeRootKey.y << levelWithinSubtree);
        var shiftZ = (subtreeRootKey.z << levelWithinSubtree);
        ...
        var dimOnLevel = (1 << levelWithinSubtree);
        var dimOnLevelSqrd = dimOnLevel * dimOnLevel;

        var arraySizes = this._unpackedArraySizes;

        // Update the bit that corresponds to this rel subtree key (d, x, y, z)
        var indexOffsetToFirstByteOnLevel = arraySizes[levelWithinSubtree];
    ...
    Cesium3DTilesetImplicit.prototype.unpackSubtreeBits = function(subtreeArrayBuffer) {
        ...
        var idx = 0;
        subtree[idx++] = payload[0];
        // second level is only 1 byte and only has 4 bits in quad tree
        if (!isOct) {
            for (i = 0; i < 4; i++) {
                subtree[idx++] = (payload[1] >> i) & 1;
            }
    ...
    Cesium3DTilesetImplicit.prototype.updateTilesetFromSubtreeNoSubtreeInfo = function(resource, subtreeArrayBuffer, parentTile) {
        ...
        while(stack.length > 0) {
            ...
            // for z, for y, for x, see if child key exists, if so create and push as child and push to stack
            treeKey = tile.treeKey;
            var childBaseX = treeKey.x << 1;
            var childBaseY = treeKey.y << 1;
            var childBaseZ = treeKey.z << 1;
            var childW = treeKey.w + 1;
            var zEnd = isOct ? 1 : 0;
            for (z = 0; z <= zEnd; z++) {
                for (y = 0; y <= 1; y++) {
                    for (x = 0; x <= 1; x++) {
                        var childX = childBaseX + x;
