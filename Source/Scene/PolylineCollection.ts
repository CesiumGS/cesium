define([
    ...
    function destroyPolylines(collection) {
        ...
        class VertexArrayBucketLocator {
            constructor(count, offset, bucket) {
                this.count = count;
                this.offset = offset;
                this.bucket = bucket;
            }
        ...
        class PolylineBucket {
            constructor(material, mode, modelMatrix) {
                this.polylines = [];
            ...
            updateShader(context, batchTable, useHighlightColor) {
            ...
            getPolylinePositionsLength(polyline) {
                var length;
            ...
            write(positionArray, texCoordExpandAndBatchIndexArray, positionIndex, colorIndex, texCoordExpandAndBatchIndexIndex, batchTable, context, projection) {
                var mode = this.mode;
            ...
            writeForMorph(positionArray, positionIndex) {
            ...
            updateIndices(totalIndices, vertexBufferOffset, vertexArrayBuckets, offset) {
            ...
            getPolylineStartIndex(polyline) {
                var polylines = this.polylines;
                var positionIndex = 0;
                var length = polylines.length;
                for (var i = 0; i < length; ++i) {
            ...
            getSegments(polyline, projection) {
            ...
            writeUpdate(index, polyline, positionBuffer, projection) {
                var mode = this.mode;
                var maxLon = projection.ellipsoid.maximumRadius * CesiumMath.PI;
