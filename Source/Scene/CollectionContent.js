...
define([
     ...
     * @private
     */
    var CollectionData = function(size) {
        this._batchSize = defaultValue(size, 0);
    ...
    defineProperties(CollectionData.prototype, {
        /**
         * DOC_TBA
         *
         * @memberof CollectionData.prototype
         *
         * @type {Number}
         * @readonly
         */
    ...
    CollectionData.prototype.setShow = function(batchId, value) {
        var batchSize = this._batchSize;
    ...
    CollectionData.prototype.getShow = function(batchId) {
        var batchSize = this._batchSize;
        //>>includeStart('debug', pragmas.debug);
        if (!defined(batchId) || (batchId < 0) || (batchId > batchSize)) {
            throw new DeveloperError('batchId is required and between zero and batchSize - 1 (' + batchSize - + ').');
    ...
    CollectionData.prototype.setColor = function(batchId, value) {
        var batchSize = this._batchSize;
        //>>includeStart('debug', pragmas.debug);
        if (!defined(batchId) || (batchId < 0) || (batchId > batchSize)) {
            throw new DeveloperError('batchId is required and between zero and batchSize - 1 (' + batchSize - + ').');
    ...
    CollectionData.prototype.setAllColor = function(value) {
        //>>includeStart('debug', pragmas.debug);
        if (!defined(value)) {
            throw new DeveloperError('value is required.');
        }
    ...
    CollectionData.prototype.getColor = function(batchId, color) {
        var batchSize = this._batchSize;
        //>>includeStart('debug', pragmas.debug);
        if (!defined(batchId) || (batchId < 0) || (batchId > batchSize)) {
            throw new DeveloperError('batchId is required and between zero and batchSize - 1 (' + batchSize - + ').');
    ...
    CollectionData.prototype.hasProperty = function(name) {
        //>>includeStart('debug', pragmas.debug);
        if (!defined(name)) {
    ...
    CollectionData.prototype.getPropertyNames = function() {
        var names = [];
        var batchTable = this._batchTable;
    ...
    CollectionData.prototype.getProperty = function(batchId, name) {
        var batchSize = this._batchSize;
        //>>includeStart('debug', pragmas.debug);
        if (!defined(batchId) || (batchId < 0) || (batchId > batchSize)) {
            throw new DeveloperError('batchId is required and between zero and batchSize - 1 (' + batchSize - + ').');
    ...
    CollectionData.prototype.setProperty = function(batchId, name, value) {
        var batchSize = this._batchSize;
        //>>includeStart('debug', pragmas.debug);
        if (!defined(batchId) || (batchId < 0) || (batchId > batchSize)) {
            throw new DeveloperError('batchId is required and between zero and batchSize - 1 (' + batchSize - + ').');
    ...
    CollectionData.prototype.modifyVertexShader = function(source) {
        ...
        return renamedSource + '\n' + getGlslComputeSt(this) + newMain;
    };

    CollectionData.prototype.modifyFragmentShader = function(source) {
        var renamedSource = ShaderSource.replaceMain(source, 'gltf_main');
        var newMain;
    ...
    CollectionData.prototype.modifyUniformMap = function(uniformMap) {
        var that = this;
        var batchUniformMap = {
    ...
    CollectionData.prototype.modifyPickVertexShader = function(source) {
        var renamedSource = ShaderSource.replaceMain(source, 'gltf_main');
        var newMain;

        if (ContextLimits.maximumVertexTextureImageUnits > 0) {
            // When VTF is supported, perform per batched model (e.g., building) show/hide in the vertex shader
            newMain =
                'uniform sampler2D tiles3d_batchTexture; \n' +
                'varying vec2 tiles3d_modelSt; \n' +
                'void main() \n' +
    ...
    CollectionData.prototype.modifyPickFragmentShader = function(source) {
        var renamedSource = ShaderSource.replaceMain(source, 'gltf_main');
        var newMain;

        if (ContextLimits.maximumVertexTextureImageUnits > 0) {
            // When VTF is supported, per batched model (e.g., building) show/hide already
            // happened in the fragment shader
            newMain =
    ...
    CollectionData.prototype.modifyPickUniformMap = function(uniformMap) {
        var that = this;
        var batchUniformMap = {
            tiles3d_batchTexture : function() {
                return defaultValue(that._batchTexture, that._defaultTexture);
    ...
    function createPickTexture(content, context) {
        ...
        if (!defined(content._pickTexture) && (batchSize > 0)) {
            ...
            for (var i = 0; i < batchSize; ++i) {
                var pickId = context.createPickId({id : i}); // TODO : change this
                pickIds.push(pickId);
    ...
    CollectionData.prototype.update = function(context, frameState) {
    ...
    CollectionData.prototype.isDestroyed = function() {
        return false;
    };

    CollectionData.prototype.destroy = function() {
