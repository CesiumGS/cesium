...
define([
     ...
     * Specifying this option will due the same, allowing for loading of these incorrect tilesets.
     *
    ...
    function createTileMapServiceImageryProvider(options) {
        ...
        function metadataSuccess(xml) {
            ...
            if (!defined(rectangle)) {
                var sw;
                var ne;
                var swXY = new Cartesian2(parseFloat(bbox.getAttribute('minx')), parseFloat(bbox.getAttribute('miny')));
                var neXY = new Cartesian2(parseFloat(bbox.getAttribute('maxx')), parseFloat(bbox.getAttribute('maxy')));
                ...
                // 'global-mercator' and 'global-geodetic' profiles.
                //
                // In the gdal2Tiles case, X and Y are always in geodetic degrees. In older versions of gdal (and only,
                // gdal) they were also flipped, which is why we check for an option to flip the values here as well.
                // Unfortunately there is no way to autodetect whether flipping is needed or not.
                var isGdal2tiles = tilingSchemeName === 'geodetic' || tilingSchemeName === 'mercator';
                var flipXY = defaultValue(options.flipXY, false);
                if (isGdal2tiles && flipXY) {
                    sw = Cartographic.fromDegrees(swXY.y, swXY.x);
                    ne = Cartographic.fromDegrees(neXY.y, neXY.x);
                } else if (tilingScheme instanceof GeographicTilingScheme || isGdal2tiles) {
