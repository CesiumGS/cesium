...
Cesium3DTile.prototype.initializeCredits = function () {
  const hasEmptyContent = this.hasEmptyContent;
  ...
  const parentCredits = defined(parent) ? parent.credits : undefined;
  ...
  const attribution = metadata.getPropertyBySemantic(
  ...
  const attributionLength = attribution.length;
  const credits = new Array(attributionLength);
  for (let i = 0; i < attributionLength; ++i) {
