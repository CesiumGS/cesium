...
/**
 * Cache for external files referenced by glTF models.
 * <p>
 * Calls to {@link GltfCache#getJson}, {@link GltfCache#getBuffer},
 * and {@link GltfCache#getTexture} are reference counted. Make sure to call
 * {@link GltfCache#releaseCacheItem} when the item is no longer needed.
 * </p>
 *
 * @alias GltfCache
 * @constructor
 ...
 * @private
 */
GltfCache.prototype.getJson = function (options) {
 ...
 * @private
 */
GltfCache.prototype.getBuffer = function (options) {
  options = defaultValue(options, defaultValue.EMPTY_OBJECT);
 ...
 * @private
 */
GltfCache.prototype.getTexture = function (options) {
  options = defaultValue(options, defaultValue.EMPTY_OBJECT);
  var gltfContainer = options.gltfContainer;
 ...
 * @private
 */
GltfCache.prototype.releaseCacheItem = function (cacheItem) {
...
function getExternalResource(gltfCache, uri) {
  var resource = gltfCache._resource;
  return resource.getDerivedResource({
    url: uri,
  });
}

function getCacheItem(gltfCache, cacheKey, childCacheItem, fetchCallback) {
  var cache = gltfCache._cache;
  var promiseCache = gltfCache._promiseCache;
