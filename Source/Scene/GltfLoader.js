...
function postProcessGeometry(loader, context) {
  ...
  for (let i = 0; i < length; i++) {
    ...
    // The glTF loader takes ownership of any attributes generated by the
    // post process stage.
    const primitive = loadPlan.primitive;
    if (defined(primitive.outlineCoordinates)) {
      loader._generatedAttributes.push(primitive.outlineCoordinates);
    }
  }
}
...
function unloadGeneratedAttributes(loader) {
  const attributes = loader._generatedAttributes;
  const length = attributes.length;
  for (let i = 0; i < length; i++) {
    const attribute = attributes[i];
    const buffer = attribute.buffer;
    if (defined(buffer) && !buffer.isDestroyed()) {
      buffer.destroy();
    }
  }
  attributes.length = 0;
