...
define([
    ...
    function GoogleEarthEnterpriseImageryProvider(options) {
        ...
        this._credit = new Credit('Google Imagery', GoogleEarthEnterpriseImageryProvider._logoData, 'http://www.google.com');
        ...
        if (!defined(this._tileDiscardPolicy)) {
            this._tileDiscardPolicy = new DiscardMissingTileImagePolicy({
                ...
                missingImageUrl : buildImageUrl(this, GoogleEarthEnterpriseImageryProvider.tileXYToQuadKey(0, 0, 1)),
    ...
    defineProperties(GoogleEarthEnterpriseImageryProvider.prototype, {
         ...
         * called before {@link GoogleEarthEnterpriseImageryProvider#ready} returns true.
         * @memberof GoogleEarthEnterpriseImageryProvider.prototype
         * @type {Boolean}
         */
        hasWaterMask : {
         ...
         * This function should not be called before {@link GoogleEarthEnterpriseImageryProvider#ready} returns true.
         * @memberof GoogleEarthEnterpriseImageryProvider.prototype
         * @type {Boolean}
         */
        hasVertexNormals : {
         ...
         * {@link GoogleEarthEnterpriseImageryProvider#ready} returns true.  This property may be undefined if availability
    ...
    GoogleEarthEnterpriseImageryProvider.prototype.requestImage = function(x, y, level) {
        ...
        var quadKey = GoogleEarthEnterpriseImageryProvider.tileXYToQuadKey(x, y, level);
                ...
                if (exists) {
                    ...
                    if (info.bits & imageBitmask) {
                        ...
                        return loadArrayBuffer(url)
                            .then(function(image) {
                                if (defined(image)) {
                                    GoogleEarthEnterpriseImageryProvider._decode(image);
     ...
     *
     * @see GoogleEarthEnterpriseImageryProvider#quadKeyToTileXY
     */
    GoogleEarthEnterpriseImageryProvider.tileXYToQuadKey = function(x, y, level) {
        var quadkey = '';
     ...
     *
     * @see GoogleEarthEnterpriseImageryProvider#tileXYToQuadKey
     */
    GoogleEarthEnterpriseImageryProvider.quadKeyToTileXY = function(quadkey) {
        var x = 0;
        var y = 0;
     ...
     * {@link GoogleEarthEnterpriseImageryProvider#ready} returns true.  The result must include terrain data and
     ...
     *
     * @exception {DeveloperError} This function must not be called before {@link GoogleEarthEnterpriseImageryProvider#ready}
     *            returns true.
     */
    GoogleEarthEnterpriseImageryProvider.prototype.requestTileGeometry = function(x, y, level, throttleRequests) {
        //>>includeStart('debug', pragmas.debug)
        if (!this._ready) {
     ...
     */
    GoogleEarthEnterpriseImageryProvider.prototype.getLevelMaximumGeometricError = function(level) {
     ...
     */
    GoogleEarthEnterpriseImageryProvider.prototype.getTileDataAvailable = function(x, y, level) {
        return undefined;
    };
    ...
    var keyBuffer;
    GoogleEarthEnterpriseImageryProvider._decode = function(data) {
        if (!defined(data)) {
    ...
    GoogleEarthEnterpriseImageryProvider.prototype._getQuadTreePacket = function(quadKey, version) {
        quadKey = defaultValue(quadKey, '');
        version = Math.max(1, defaultValue(version, 1));
        ...
        return loadArrayBuffer(url)
            .then(function(metadata) {
                GoogleEarthEnterpriseImageryProvider._decode(metadata);
                ...
                var compressedPacket = new Uint8Array(metadata, offset);
                ...
                dv = new DataView(uncompressedPacket.buffer);
                offset = 0;
                magic = dv.getUint32(offset, true);
                offset += Uint32Size;
                if (magic !== qtMagic) {
                    throw new RuntimeError('Invalid magic');
                }
                ...
                if (dataBufferOffset + dataBufferSize + metaBufferSize !== size) {
                ...
                for (var i = 0; i < numInstances; ++i) {
                    ...
                    offset += Uint16Size; // 4 byte align
    ...
    function populateSubtree(that, quadKey) {
        ...
        promise = that._getQuadTreePacket(q, t.cnodeVersion);
