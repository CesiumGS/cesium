...
function spin3D(controller, startPosition, movement) {
  ...
  if (defined(globe) && height < controller._minimumPickingTerrainHeight) {
    if (defined(mousePos)) {
      var ray2 = camera.getPickRay(movement.startPosition, pickGlobeScratchRay);
      var angleLerp = Cartesian3.dot(up, ray2.direction);
      var strafing = false;

      if (cameraUnderground) {
        strafing = angleLerp > 0.0;
      } else {
        strafing =
...
function getTiltCenterUnderground(
  controller,
  ray,
  height,
  pickedPosition,
  ...
  // Only allow the tilt center to be so far away from the camera, dependent
  // on the max distance and how close the camera is to the surface
...
function tilt3DOnTerrain(controller, startPosition, movement) {
  ...
  if (Cartesian2.equals(startPosition, controller._tiltCenterMousePosition)) {
    ...
    if (scene.cameraUnderground) {
      ...
      getTiltCenterUnderground(controller, ray, height, center, center);
    }
