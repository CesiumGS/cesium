...
import TweenCollection from './TweenCollection.js';
    ...
    function handleZoom(object, startPosition, movement, zoomFactor, distanceMeasure, unitPositionDotDirection) {
        ...
        if (!sameStartPosition || rotatingZoom) {
            if (mode === SceneMode.SCENE2D) {
                ...
                if (object._zoomingOnVectorUnderground || scene.frameState.cameraUnderground || camera.positionCartographic.height < 3000.0 && Math.abs(Cartesian3.dot(camera.direction, cameraPositionNormal)) < 0.6) {
                    zoomOnVector = true;
                } else {
    ...
    function getBackwardsDistance(controller, ray) {
        ...
        if (controller._ellipsoid !== controller._originalEllipsoid) {
            ...
            controller._expandedEllipsoid = new Ellipsoid(radii.x * expansionFactor, radii.y * expansionFactor, radii.z * expansionFactor);
        }
    ...
    function zoom3D(controller, startPosition, movement) {
        ...
        var sameStartPosition = Cartesian2.equals(startPosition, controller._zoomMouseStart);

        if (!sameStartPosition) {
            controller._zoomingOnVectorUnderground = scene.frameState.cameraUnderground && defined(intersection);
        }
