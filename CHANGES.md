...
- Fixed a `Primitive` rendering inside-out when its `modelMatrix` is a mirroring transform, i.e. it has a negative determinant. The triangle winding order is now reversed when geometry is baked to world coordinates, so back face culling keeps culling the back faces of the mirrored geometry.
