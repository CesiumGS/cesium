...
* Breaking changes ([why so many?](https://groups.google.com/forum/#!topic/cesium-dev/Y_mG11IZD9k))
  ...
  * CZML changes:
    * CZML is now versioned using the <major>.<minor> scheme.  For example, any CZML 1.0 implementation will be able to load any 1.<minor> document (with graceful degradation).  Major version number increases will be reserved for breaking changes.  We full expect these major version increases to happen, as CZML is still in development, but we wanted to give developers a stable target to work with.
    * A `1.0` version string is required to be on the document packet of all objects.  Previously a `document` packet was optional; it is now mandatory.  The simplest document packet is: 
      ```
      {
        "id":"document",
        "version":"1.0"
    ...
    * The `orientation` property has been changed to match Cesium convention.  So update existing CZML documents; simply conjugate the quaternion values.
    * `pixelOffset` now treats `y` as the top of the screen, previously it was the bottom.  To update simply negate the `y` value of existing CZML documents.
