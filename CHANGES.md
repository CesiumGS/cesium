...
* Breaking changes
  ...
  * `PolygonGeometry` throws a `DeveloperError` when using `options.perPositionHeight === true` and `options.height !== undefined`.  Remove `options.height` to fix the error (it is not used in the polygon geometry calculation)
* Deprecated
  *
...
* Fixed vertex normals for a `PolygonGeometry` using `options.perPositionHeight`
