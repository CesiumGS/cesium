...
* Added `Cesium3DTileset.tileFailed` event that will be raised when a tile fails to load. The object passed to the event listener will have a url and message property. If there are no event listeners, error message will be logged to the console. [#6088](https://github.com/AnalyticalGraphicsInc/cesium/pull/6088)
