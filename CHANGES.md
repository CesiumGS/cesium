...
- Fixed a bug where `TaskProcessor` worker loading would check the worker module name rather than the absolute URL when determining if it is cross-origin.
- Fixed a bug where cross-origin workers were loaded using the CommonJS `importScripts` shim instead of an ESM `import`.
