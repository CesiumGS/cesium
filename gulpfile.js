...
export async function deployS3() {
  ...
  const argv = yargs(process.argv)
    ...
    .options({
      ...
      "only-coverage": {
        description: "Only deploy coverage files.",
        type: "boolean",
        default: false,
      },
    }).argv;
  ...
  if (argv.onlyCoverage) {
    return deployCoverage(bucketName, uploadDirectory, cacheControl, dryRun);
  }
...
async function deployCoverage(
  ...
  if (isProduction) {
    ...
    const files = await globby(["Build/Coverage"], {
