...
- [Coding Guide](#coding-guide)
  ...
  - [Classes](#classes)
    ...
    - [Put the Class at the Top of the File](#put-the-class-at-the-top-of-the-file)
  - [Design](#design)
...
When referencing external documentation and examples — including TypeScript documentation — keep in mind that the TypeScript toolchain supports an extended syntax that diverges from the official JSDoc standard. The CesiumJS documentation, generated by [tsd-jsdoc](https://github.com/englercj/tsd-jsdoc/), supports only plain JSDoc, pending resolution of [#10455](https://github.com/CesiumGS/cesium/issues/10455). Take care to use valid JSDoc (not TypeScript-flavored JSDoc!) as the type system and the documentation rely on the same declarations. In rare cases, it may also be possible to configure tsd-jsdoc to safely ignore a specific unsupported syntax.
...
2. **Common JavaScript/JSDoc obstacles.** JSDoc-based type checks have limited support for some coding patterns used in the CesiumJS codebase, such as classic prototype-based inheritance and `Object.defineProperties()`. In most cases these problems can be solved by refactoring with newer coding patterns. In other cases the solution may be less obvious, and a temporary skip-check annotation may be used to satisfy the type system. For current recommendations and tooling for migrating to ES6 `class` syntax, see [Classes](#classes) below, and [#8359](https://github.com/CesiumGS/cesium/issues/8359).
...
5. **Type-only imports.** When JSDoc annotations depend on types not otherwise imported in a source file, it will be necessary to tell TypeScript where to find them. To avoid otherwise-unused imports, use type-only imports in separate one-line JSDoc comments:
   ...
   ```
   > _WARNING: An alternative type import syntax, `/** @typedef {import('./File1.js')} MyObject1 */`, may break generation of documentation using tsd-jsdoc, and must be avoided._
6. **Prefer inferred types where available.** TypeScript can infer types from variable initialization. Add JSDoc to internal variables only when needed to provide types or to refine inferred types. For public APIs, explicit JSDoc types are required for documentation. We enable [`noImplicitAny`](https://www.typescriptlang.org/tsconfig/#noImplicitAny) in `tsconfig.json`, so that TypeScript will complain when explicit types are missing, rather than silently using the `any` type. Function return types are a common exception to this rule — rather than letting TypeScript infer return types from the function's implementation, it's often worthwhile to explicitly declare the return type for non-trivial functions, to prevent inadvertly returning a previously-unsupported value and changing the inferred types.
   ```javascript
   const x = "hello world"; // good
   const x = /** @type {string} */ ("hello world"); // bad
   ```
...
Use the functions of Cesium's [Check](https://github.com/CesiumGS/cesium/blob/main/packages/engine/Source/Core/Check.js) class to throw a `DeveloperError` when the user has a coding error. The most common errors are parameters that are missing, have the wrong type or are out of range.
...
- Create classes using [ES6 `class` syntax](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Classes):
...
Older classes in the codebase written with prototype-based inheritance, and help with converting these to ES6 classes is welcome. For tooling and recommendations on these conversions, see [#8359](https://github.com/CesiumGS/cesium/issues/8359).
...
- :speedboat: Assign to all the property members of a class in the constructor or class body. This allows JavaScript engines to use a hidden class and avoid entering dictionary mode. Assign `undefined` if no initial value makes sense. Do not add properties to an object, e.g.,
...
- In a constructor, consider properties as write once; do not write to them or read them multiple times. Create a local variable if they need to be read. For example:
...
Since calling a `from` constructor should not require an existing object, the function is static.
...
The static functions have the benefit of being able to be used polymorphically.
...
Public properties that can be read or written without extra processing can simply be assigned in the constructor, e.g.,
...
Read-only properties can be created with a private property and a getter:
...
### Put the Class at the Top of the File

It is convenient for the class body to be at the top of the file, even if it requires that helper functions rely on **hoisting**, for example, `Cesium3DTileset.js`,
...
- :art: Put new classes and functions in the right part of the Cesium stack (directory). From the bottom up:
  - `packages/engine/Source/Core` - Number crunching. Pure math such as [`Cartesian3`](https://github.com/CesiumGS/cesium/blob/main/packages/engine/Source/Core/Cartesian3.js). Pure geometry such as [`CylinderGeometry`](https://github.com/CesiumGS/cesium/blob/main/packages/engine/Source/Core/CylinderGeometry.js). Fundamental algorithms such as [`mergeSort`](https://github.com/CesiumGS/cesium/blob/main/packages/engine/Source/Core/mergeSort.js).
  - `packages/engine/Source/Renderer` - WebGL abstractions such as [`ShaderProgram`](https://github.com/CesiumGS/cesium/blob/main/packages/engine/Source/Renderer/ShaderProgram.js) and WebGL-specific utilities such as [`ShaderCache`](https://github.com/CesiumGS/cesium/blob/main/packages/engine/Source/Renderer/ShaderCache.js). Identifiers in this directory are not part of the public Cesium API.
  - `packages/engine/Source/Scene` - The graphics engine, including primitives such as [`Model`](https://github.com/CesiumGS/cesium/blob/main/packages/engine/Source/Scene/Model/Model.js). Code in this directory often depends on `Renderer`.
  - `packages/engine/Source/DataSources` - Entity API, such as [`Entity`](https://github.com/CesiumGS/cesium/blob/main/packages/engine/Source/DataSources/Entity.js), and data sources such as [`CzmlDataSource`](https://github.com/CesiumGS/cesium/blob/main/packages/engine/Source/DataSources/CzmlDataSource.js).
  - `packages/widgets/Source` - Widgets such as the main Cesium [`Viewer`](https://github.com/CesiumGS/cesium/blob/main/packages/widgets/Source/Viewer/Viewer.js).

It is usually obvious what directory a file belongs in. When it isn't, the decision is usually between `Core` and another directory. Put the file in `Core` if it is pure number crunching or a utility that is expected to be generally useful to Cesium, e.g., [`Matrix4`](https://github.com/CesiumGS/cesium/blob/main/packages/engine/Source/Core/Matrix4.js) belongs in `Core` since many parts of the Cesium stack use 4x4 matrices; on the other hand, [`BoundingSphereState`](https://github.com/CesiumGS/cesium/blob/main/packages/engine/Source/DataSources/BoundingSphereState.js) is in `DataSources` because it is specific to data sources.
...
- Use [`deprecationWarning`](https://github.com/CesiumGS/cesium/blob/main/packages/engine/Source/Core/deprecationWarning.js) to warn users that the API is deprecated and what proactive changes they can take, e.g.,
...
Use a knockout subscription only when you are unable to accomplish what you need to do with a standard binding. For [example](https://github.com/CesiumGS/cesium/blob/main/packages/widgets/Source/Viewer/Viewer.js), the `Viewer` subscribes to `FullscreenButtonViewModel.isFullscreenEnabled` because it needs to change the width of the timeline widget when that value changes. This cannot be done with binding because the value from `FullscreenButtonViewModel` is affecting a value not contained within that widget.

Cesium includes a [`subscribeAndEvaluate`](https://github.com/CesiumGS/cesium/blob/main/packages/widgets/Source/subscribeAndEvaluate.js) helper function for subscribing to knockout observable.

When using a subscription, always be sure to [dispose the subscription](https://github.com/CesiumGS/cesium/blob/main/packages/widgets/Source/Viewer/Viewer.js) when the viewmodel is no longer using it. Otherwise the listener will continue to be notified for the lifetime of the observable.
