(function () {
    ...
    describe("Cache", function () {
        ...
        it("constructor throws when policy has no hit function", function () {
            expect(function () {
                return new Cache({ miss : function () { return "nonsense"; } });
            }).toThrow();
        });

        it("constructor throws when policy has no miss function", function () {
            expect(function () {
                return new Cache({ hit : function () { return "nonsense"; } });
            }).toThrow();
        });
        ...
        it("find throws with no key", function () {
            ...
            expect(function () {
                cache.find({ nonsense : "nonsense" });
            }).toThrow();
        });

        it("find", function () {
            var tenth = { n : 10, key : "10" };
            var fn = cache.find(tenth);
            expect(fn.fibonacciNumber).toEqual(55);

            var eleventh = { n : 11, key : "11" };
        ...
        it("remove", function () {
            ...
            expect(cache.remove({ n : 14, key : "14" })).toBeFalsy();
        });
    });
}());
