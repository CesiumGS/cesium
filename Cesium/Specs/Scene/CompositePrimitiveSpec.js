(function() {
    ...
    describe("CompositePrimitive", function () {
        ...
        beforeEach(function () {
            ...
            primitives = new Cesium.CompositePrimitive();

            camera = new Cesium.Camera(context.getCanvas());
            camera.position = new Cesium.Cartesian3(1.02, 0.0, 0.0);
            camera.up = Cesium.Cartesian3.getUnitZ();
            camera.direction = camera.position.negate();
            camera.frustum.near = 0.01;
            camera.frustum.far = 10.0;
            camera.frustum.fovy = Cesium.Math.toRadians(60.0);
            camera.frustum.aspectRatio = 1.0;

            us = context.getUniformState();
            us.setView(camera.getViewMatrix());
            us.setProjection(camera.frustum.getProjectionMatrix());
            us.setSunPosition(new Cesium.Cartesian3(-2.0, 0.0, 0.0));
        });
        ...
        it("adds and removes a primitive in two composites", function () {
            ...
            var otherPrimitives = new Cesium.CompositePrimitive(context);
        ...
        it("setting a central body", function () {
            var ellipsoid = Cesium.Ellipsoid.getUnitSphere();
            var cb = new Cesium.CentralBody(camera, ellipsoid);
        ...
        it("renders a central body", function () {
            ...
            var cb = new Cesium.CentralBody(camera, Cesium.Ellipsoid.getUnitSphere());
            primitives.setCentralBody(cb);

            primitives.update(context, Cesium.Specs.sceneState);
        ...
        it("renders a primitive in more than one composite", function () {
            ...
            otherPrimitives.update(context, Cesium.Specs.sceneState);
        ...
        it("picks a primitive added with add()", function () {
            ...
            var pickedObject = Cesium.Specs.pick(context, primitives, 0, 0);
        ...
        it("is destroyed after calling destroy()", function () {
            var p = new Cesium.CompositePrimitive();
            p.destroy();
            expect(p.isDestroyed()).toBeTruthy();
        });

        it("destroys its primitives", function () {
            var labels = new Cesium.LabelCollection(context);
