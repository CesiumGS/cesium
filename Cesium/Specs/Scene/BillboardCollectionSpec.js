(function() {
    "use strict";
    /*global Cesium, Image, describe, it, expect, beforeEach, afterEach, waitsFor*/

    describe("BillboardCollection", function () {
        // TODO: rendering tests for pixel offset, eye offset, horizontal origin, vertical origin
        ...
        it("explicitly constructs a billboard", function () {
            var b = billboards.add({
                ...
                pixelOffset : new Cesium.Cartesian2(1.0, 2.0),
                eyeOffset : new Cesium.Cartesian3(1.0, 2.0, 3.0),
                horizontalOrigin : Cesium.HorizontalOrigin.LEFT,
                verticalOrigin : Cesium.VerticalOrigin.BOTTOM,
                scale : 2.0,
                imageIndex : 1,
                color : { red : 1.0, green : 2.0, blue : 3.0, alpha : 4.0 }
            });
        ...
        it("set's a billboard's properties", function () {
            ...
            b.setPixelOffset(new Cesium.Cartesian2(1.0, 2.0));
            b.setEyeOffset(new Cesium.Cartesian3(1.0, 2.0, 3.0));
            b.setHorizontalOrigin(Cesium.HorizontalOrigin.LEFT);
            b.setVerticalOrigin(Cesium.VerticalOrigin.BOTTOM);
            b.setScale(2.0);
            b.setImageIndex(1);
            b.setColor({ red : 1.0, green : 2.0, blue : 3.0, alpha : 4.0 });
        ...
        it("doesn't render when constructed", function () {
            ...
            billboards.update(context, Cesium.Specs.sceneState);
        ...
        it("adds and renders a billboard", function () {
            ...
            billboards.add({
                position : { x : -0.5, y : 0.0, z : 0.0 },      // Closer to viewer
                imageIndex : 1
            });
        ...
        it("renders using billboard's position property", function () {
            ...
            b.setPosition({ x : -2.0, y : 0.0, z : 0.0 });  // Behind viewer
            billboards.update(context, Cesium.Specs.sceneState);
            billboards.render(context, us);
            expect(context.readPixels()).toEqualArray([0, 0, 0, 0]);

            b.setPosition({ x : 0.0, y : 0.0, z : 0.0 });  // Back in front of viewer
            billboards.update(context, Cesium.Specs.sceneState);
            billboards.render(context, us);
            expect(context.readPixels()).toEqualArray([0, 255, 0, 255]);
        });
        ...
        it("renders using billboard's color property", function () {
            ...
            b.setColor({ red : 1.0, green : 0.0, blue : 1.0, alpha : 1.0 });
            billboards.update(context, Cesium.Specs.sceneState);
            billboards.render(context, us);
            ...
            b.setColor({ red : 0.0, green : 1.0, blue : 0.0, alpha : 1.0 });
            billboards.update(context, Cesium.Specs.sceneState);
            billboards.render(context, us);
            expect(context.readPixels()).toEqualArray([0, 255, 0, 255]);
        });
        ...
        it("updates 10% of billboards", function () {
            ...
            billboards.get(3).setColor({ red : 0.0, green : 1.0, blue : 0.0, alpha : 1.0 });
            ...
            billboards.get(3).setColor({ red : 1.0, green : 0.0, blue : 0.0, alpha : 1.0 });
        ...
        it("is picked", function () {
            ...
            var pickedObject = Cesium.Specs.pick(context, billboards, 0, 0);
        ...
        it("computes screen space position (1)", function () {
            ...
            expect(b.computeScreenSpacePosition(us).equals(new Cesium.Cartesian2(0.5, 0.5)));
        });

        it("computes screen space position (2)", function () {
            var b = billboards.add({
                position    : { x : 0.0, y : 0.0, z : 0.0 },
                pixelOffset : { x : 1.0, y : 2.0 }
            });

            expect(b.computeScreenSpacePosition(us).equals(new Cesium.Cartesian2(1.5, 2.5)));
        });

        it("computes screen space position (3)", function () {
            var b = billboards.add({
                position  : { x : 0.0, y : 0.0, z : 0.0 },
                eyeOffset : { x : 5.0, y : 5.0, z : 0.0 }
            });
