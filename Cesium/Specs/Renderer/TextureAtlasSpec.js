(function() {
    "use strict";
    /*global Cesium, Image describe, it, expect, beforeEach, afterEach, waitsFor, Float32Array*/
    ...
    describe("TextureAtlas", function () {
        ...
        var draw = function(texture, textureCoordinate, expectedColorArray) {
            ...
            var fs = "uniform sampler2D u_texture;" +
                     "void main() { gl_FragColor = texture2D(u_texture, vec2(" + textureCoordinate.x.toString() + ", " + textureCoordinate.y.toString() + ")); }";
        ...
        it("creates a single image atlas", function () {
            atlas = context.createTextureAtlas([greenImage], Cesium.PixelFormat.RGBA, 0);
        ...
        it("renders a single image atlas", function () {
            ...
            expect(draw.apply(this, [atlas.getTexture(), { x : 0.5, y : 0.5 }])).toEqualArray([0, 255, 0, 255]);
        });

        it("creates a two image atlas", function () {
            atlas = context.createTextureAtlas([greenImage, blueImage], Cesium.PixelFormat.RGBA, 0);
            var texture = atlas.getTexture();
            var coordinates = atlas.getTextureCoordinates();
        ...
        it("renders a four image atlas", function () {
            atlas = context.createTextureAtlas([greenImage, blueImage, blueImage, greenImage], Cesium.PixelFormat.RGBA, 0);
        ...
        it("creates an atlas with different image heights", function () {
            atlas = context.createTextureAtlas([blueImage, tallGreenImage], Cesium.PixelFormat.RGBA, 0);
            var texture = atlas.getTexture();
            var coordinates = atlas.getTextureCoordinates();
        ...
        it("renders an atlas with different image heights", function () {
            ...
            expect((pixels[0] === 0) ||
                   (pixels[0] === 1)).toBeTruthy();     // Workaround:  Firefox on Windows
        ...
        it("throws with a negative borderWidthInPixels", function () {
            expect(function () {
                atlas = context.createTextureAtlas([], Cesium.PixelFormat.RGBA, -1);
            }).toThrow();
        });
    });
}());