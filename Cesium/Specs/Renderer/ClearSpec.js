(function() {
    ...
    describe("Clear", function () {
        ...
        it("clears a framebuffer color attachment", function () {
            ...
            var framebuffer = context.createFramebuffer({ colorTexture : colorTexture });
            ...
            expect(context.readPixels({ framebuffer : framebuffer })).toEqualArray([0, 255, 0, 255]);
        ...
        it("fails to read pixels (width)", function () {
            expect(function () {
                expect(context.readPixels({ width : -1 })).toEqualArray([0, 0, 0, 0]);
            }).toThrow();        
        });            
        
        it("fails to read pixels (height)", function () {
            expect(function () {
                expect(context.readPixels({ height : -1 })).toEqualArray([0, 0, 0, 0]);
            }).toThrow();        
        });              
    });
}());