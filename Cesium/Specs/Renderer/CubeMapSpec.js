(function() {
    ...
    describe("Cube Map", function () {
        ...
        it("creates with defaults", function() {
            cubeMap = context.createCubeMap({ width : 16, height : 16 });

            expect(cubeMap.getPixelFormat()).toEqual(Cesium.PixelFormat.RGBA);
            expect(cubeMap.getPixelDatatype()).toEqual(Cesium.PixelDatatype.UNSIGNED_BYTE);
        });
        ...
        it("gets the default sampler", function () {
            ...
            expect(sampler.wrapS).toEqual(Cesium.TextureWrap.CLAMP);
            expect(sampler.wrapT).toEqual(Cesium.TextureWrap.CLAMP);
            expect(sampler.minificationFilter).toEqual(Cesium.TextureMinificationFilter.LINEAR);
            expect(sampler.magnificationFilter).toEqual(Cesium.TextureMagnificationFilter.LINEAR);
        });
        ...
        it("draws with a cube map", function () {
            ...
            var fs = "uniform samplerCube u_texture;" +
            ...
            // +X is blue
            sp.getAllUniforms().u_direction.value = new Cesium.Cartesian3(1, 0, 0);
            context.draw(da);
            ...
            // +Y is blue
            sp.getAllUniforms().u_direction.value = new Cesium.Cartesian3(0, 1, 0);
            context.draw(da);
            expect(context.readPixels()).toEqualArray([0, 0, 255, 255]);
            ...
            // +Z is blue
            sp.getAllUniforms().u_direction.value = new Cesium.Cartesian3(0, 0, 1);
            context.draw(da);
            expect(context.readPixels()).toEqualArray([0, 0, 255, 255]);
        ...
        it("copies to a cube map", function () {
            cubeMap = context.createCubeMap({ width : 1, height : 1 });
        ...
        it("draws with a cube map and a texture", function () {
            ...
            var texture = context.createTexture2D({ source : blueImage });
        ...
        it("destroys", function () {
            var c = context.createCubeMap({ width : 16, height : 16 });
        ...
        it("fails to create (width != height)", function(){
            expect(function() {
                cubeMap = context.createCubeMap({ width : 16, height : 32 });
            }).toThrow();
        });
        ...
        it("fails to generate mipmaps (width)", function () {
            cubeMap = context.createCubeMap({ width : 3, height : 3 });
