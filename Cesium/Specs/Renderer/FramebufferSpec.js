(function() {
    "use strict";
    /*global Cesium, describe, it, expect, beforeEach, afterEach, waitsFor, Float32Array, Uint16Array, ArrayBuffer*/
    ...
    describe("Framebuffer", function () {
        ...
        it("has a color attachment", function () {
            var colorTexture = context.createTexture2D({ width : 1, height : 1 });
        ...
        it("has a depth attachment", function () {
            var renderbuffer = context.createRenderbuffer({ format : Cesium.RenderbufferFormat.DEPTH_COMPONENT16 });
        ...
        it("has a stencil attachment", function () {
            var renderbuffer = context.createRenderbuffer({ format : Cesium.RenderbufferFormat.Stencil8 });
        ...
        it("has a depth-stencil attachment", function () {
            var renderbuffer = context.createRenderbuffer({ format : Cesium.RenderbufferFormat.DEPTH_STENCIL });
        ...
        it("clears a color attachment", function() {
            ...
            var colorTexture = context.createTexture2D({ width : 1, height : 1 });
            framebuffer = context.createFramebuffer({ colorTexture : colorTexture });
            ...
            sp = context.createShaderProgram(vs, fs, { position : 0});
        ...
        it("clears a cube map face color attachment", function() {
            var cubeMap = context.createCubeMap({ width : 1, height : 1 });
            ...
            // 2 of 4.  Clear framebuffer color attachment to green.
            framebuffer = context.createFramebuffer({ colorTexture : cubeMap.getPositiveX() });
            ...
            // 4 of 4.  Render green to default color buffer by reading from previous color attachment
            var vs = "attribute vec4 position; void main() { gl_PointSize = 1.0; gl_Position = position; }";
            var fs = "uniform samplerCube u_cubeMap;" +
        ...
        it("draws to a color attachment", function() {
            ...
            var fs2 = "uniform sampler2D u_texture;" +
                     "void main() { gl_FragColor = texture2D(u_texture, vec2(0.0)); }";
            var sp2 = context.createShaderProgram(vs2, fs2, { position : 0});
        ...
        it("draws with a depth attachment", function() {
            framebuffer = context.createFramebuffer({ 
                colorTexture      : context.createTexture2D({ width : 1, height : 1 }),
            ...
            context.clear(context.createClearState({ framebuffer : framebuffer }));
            expect(context.readPixels()).toEqualArray([0, 0, 0, 0]);
            ...
            context.draw({
                ...
                renderState   : context.createRenderState({
                    depthTest : {
                        enabled : true,
                        func    : Cesium.DepthFunction.NEVER
                    }
                })
            });
            expect(context.readPixels({ framebuffer : framebuffer })).toEqualArray([0, 0, 0, 0]);
            ...
            expect(context.readPixels({ framebuffer : framebuffer })).toEqualArray([255, 255, 255, 255]);
        });
        ...
        it("draws with a stencil attachment", function() {
            framebuffer = context.createFramebuffer({ 
                ...
                stencilRenderbuffer : context.createRenderbuffer({
                    format : Cesium.RenderbufferFormat.STENCIL_INDEX8,
                    width  : 1,
                    height : 1
                })
            });
            ...
            context.draw({
                ...
                framebuffer   : framebuffer,
                uniformMap  : {
                    u_color : function () { return new Cesium.Cartesian4(1.0, 1.0, 1.0, 1.0); }
                },
                renderState   : context.createRenderState({
                    stencilTest : {
                        enabled        : true,
            ...
            context.draw({
                ...
                framebuffer   : framebuffer,
                uniformMap  : {
                    u_color : function () { return new Cesium.Cartesian4(0.0, 0.0, 0.0, 0.0); }
                },
                renderState   : context.createRenderState({
                    stencilTest : {
                        enabled        : true,
        ...
        it("throws when the depth test is enabled without an appropriate attachment", function() {
            ...
            expect(function () {
                context.draw({
                    ...
                    vertexArray   : va,
                    framebuffer   : framebuffer,
                    renderState   : context.createRenderState({ depthTest : { enabled : true } })
                });
            }).toThrow();        
        }); 
