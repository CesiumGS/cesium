/*global define*/
define(['./binarySearch', './TimeInterval', './JulianDate'], function(binarysearch, TimeInterval, JulianDate) {
    "use strict";
    ...
    function intersectInternal(that, intervals, dataComparer, mergeCallback){
        ...
        while (left < that._intervals.length && right < otherIntervals.length)
        {
            ...
            else
            {
                ...
                var intersection = typeof mergeCallback === 'undefined' ?
                        that._intervals[left].intersect(otherIntervals[right], dataComparer) :
                            that._intervals[left].intersectMergingData(otherIntervals[right], mergeCallback);
        ...
        if (!interval.isEmpty)
        {
            ...
            else
            {
                ...
                if (index > 0 &&
                    ...
                    this._intervals[index - 1].isStartIncluded &&
                    this._intervals[index - 1].start.equals(interval.start))
                {
                    --index;
                }
                else if (index < this._intervals.length &&
                        !interval.isStartIncluded &&
                        this._intervals[index].isStartIncluded &&
                        this._intervals[index].start.equals(interval.start))
                {
                    ++index;
                }
            }
            ...
            if (index > 0)
            {
                ...
                if (comparison > 0  ||
                    (comparison === 0 &&
                     (this._intervals[index - 1].isStopIncluded || interval.isStartIncluded)))
                {
                    // There is an overlap
                    if (typeof dataComparer !== 'undefined' ? dataComparer(this._intervals[index - 1].data, interval.data) : (this._intervals[index - 1].data === interval.data))
                    {
                        ...
                        else
                        {
                            interval = new TimeInterval(
                                this._intervals[index - 1].start,
                                this._intervals[index - 1].stop,
                                this._intervals[index - 1].isStartIncluded,
                                this._intervals[index - 1].isStopIncluded || (interval.stop.equals(this._intervals[index - 1].stop) && interval.isStopIncluded),
                                interval.data);
                        }
                    ...
                    else
                    {
                        ...
                        if (comparison > 0 ||
                            (comparison === 0 && this._intervals[index - 1].isStopIncluded && !interval.isStopIncluded))
                        {
                            this._intervals.splice(index, 0, new TimeInterval(
                                interval.stop,
                                this._intervals[index - 1].stop,
                                !interval.isStopIncluded,
                                this._intervals[index - 1].isStopIncluded,
                                this._intervals[index - 1].data));
                            this._intervals[index - 1] = new TimeInterval(
                                    this._intervals[index - 1].start,
                                interval.start,
                                this._intervals[index - 1].isStartIncluded,
                                !interval.isStartIncluded,
            ...
            while (index < this._intervals.length)
            {
                ...
                if (comparison > 0 ||
                    (comparison === 0 && (interval.isStopIncluded || this._intervals[index].isStartIncluded)))
                {
                    // There is an overlap
                    ...
                    else
                    {
                        ...
                        this._intervals[index] = new TimeInterval(
                            interval.stop,
                            this._intervals[index].stop,
                            !interval.isStopIncluded,
                            this._intervals[index].isStopIncluded,
...
//    };
    ...
    TimeIntervalCollection.prototype.indexOf = function (date) {
        ...
        if (index >= 0)
        {
            ...
            if (index > 0 &&
                this._intervals[index - 1].stop.equals(date) &&
                this._intervals[index - 1].isStopIncluded)
            {
                return index - 1;
            }
        ...
        index = ~index;
        if (index > 0 &&
            (index - 1) < this._intervals.length &&
            this._intervals[index - 1].contains(date))
        {
            return index - 1;
        }
        return ~index;
    ...
    TimeIntervalCollection.prototype.removeInterval = function(interval) {
        ...
        if (index < this._intervals.length &&
            !interval.isStartIncluded &&
            this._intervals[index].isStartIncluded &&
            interval.start.equals(this._intervals[index].start))
        {
            result = true;

            this._intervals.splice(index, 0, new TimeInterval(
                    this._intervals[index].start,
                    this._intervals[index].start,
                    true,
                    true,
                    this._intervals[index].data));
        ...
        if (index < this._intervals.length && interval.stop.equals(this._intervals[index].stop))
        {
            ...
            if (!interval.isStopIncluded && this._intervals[index].isStopIncluded)
            {
                ...
                if ((index + 1) < this._intervals.length &&
                        this._intervals[index + 1].start.equals(interval.stop) &&
                    this._intervals[index].data === this._intervals[index + 1].data)
                {
