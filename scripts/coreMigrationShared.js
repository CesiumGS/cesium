...
/**
 * Finds every file directly under the repo-root Specs/ directory that depends,
 * directly or transitively (through another root Specs/ helper), on a symbol that
 * stays in "@cesium/engine" (or anything from "@cesium/widgets", which never moves).
 * A packages/core Spec importing any of these would pull engine back into core - e.g.
 * Specs/createContext.js constructs a real Scene/Context, neither of which move.
 * Helpers whose "@cesium/engine" imports are made up entirely of symbols that *do*
 * move (e.g. only `defined`/`Math`) are not flagged, since rewriteCoreImports.js
 * rewrites those to reference "@cesium/core" instead.
 *
 * @param {Set<string>} movedSymbolNames Barrel export names moving to @cesium/core.
 * @returns {Promise<Set<string>>} Absolute paths of the engine/widgets-dependent helpers.
 */
export async function getEngineDependentRootSpecHelpers(movedSymbolNames) {
  ...
  /**
   * True if this import keeps at least one symbol that will still live in
   * "@cesium/engine" after rewriteCoreImports.js runs (or is from "@cesium/widgets",
   * which never moves anything to core).
   * @param {string} namesBlock
   * @param {string} specifier
   */
  function importsNonMovedSymbol(namesBlock, specifier) {
    if (specifier === "@cesium/widgets") {
      return true;
    }
    if (specifier !== "@cesium/engine") {
      return false;
    }
    ...
    return names.some((entry) => !movedSymbolNames.has(localName(entry)));
  }

  /** Absolute path -> other root Specs/ files it imports by relative path. */
  const localImports = new Map();
  const directlyDependent = new Set();
  ...
  for (const file of files) {
    ...
    for (const regex of [
      ...
      while ((match = regex.exec(source))) {
        const nameOrNames = match[1];
        const specifier = match[2];

        if (importsNonMovedSymbol(nameOrNames, specifier)) {
          directlyDependent.add(file);
          continue;
        }
    ...
    localImports.set(file, targets);
  }

  // Propagate dependence transitively (e.g. A imports B, B imports "@cesium/engine").
  const dependent = new Set(directlyDependent);
  let changed = true;
  while (changed) {
    changed = false;
    for (const [file, targets] of localImports) {
      if (
        !dependent.has(file) &&
        targets.some((/** @type {string} */ t) => dependent.has(t))
      ) {
        dependent.add(file);
        changed = true;
      }
