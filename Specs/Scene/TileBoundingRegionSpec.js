...
describe("Scene/TileBoundingRegion", function () {
  ...
  it("deriveVolume throws if level is undefined", function () {
    expect(function () {
      tileBoundingRegion.deriveVolume(undefined, 0, 0);
    }).toThrowDeveloperError();
  });

  it("deriveVolume throws if x is undefined", function () {
    expect(function () {
      tileBoundingRegion.deriveVolume(1, undefined, 0);
    }).toThrowDeveloperError();
  });

  it("deriveVolume throws if y is undefined", function () {
    expect(function () {
      tileBoundingRegion.deriveVolume(1, 0, undefined);
    }).toThrowDeveloperError();
  });
  ...
  it("deriveVolume subdivides like a quadtree if z is not given", function () {
    var tile = new TileBoundingRegion({
      ...
      // be 1 degree each
      rectangle: Rectangle.fromDegrees(-1, -2, 3, 2),
      minimumHeight: 0,
      maximumHeight: 20,
    });
    var expected = new TileBoundingRegion({
      rectangle: Rectangle.fromDegrees(0, 1, 1, 2),
      minimumHeight: 0,
      maximumHeight: 20,
    });

    var result = tile.deriveVolume(2, 1, 3);
    expect(result.rectangle).toEqualEpsilon(
      expected.rectangle,
      CesiumMath.EPSILON9
    );
    expect(result.minimumHeight).toEqual(expected.minimumHeight);
  ...
  it("deriveVolume subdivides like an octree if z is given", function () {
    ...
    var result = tile.deriveVolume(2, 1, 3, 2);
    expect(result.rectangle).toEqualEpsilon(
      expected.rectangle,
      CesiumMath.EPSILON9
    );
  ...
  it("handles the IDL", function () {
    ...
    var expected = new TileBoundingRegion({
      rectangle: Rectangle.fromDegrees(180, -45, -90, 0),
      minimumHeight: 0,
      maximumHeight: 20,
    });
    var result = tile.deriveVolume(1, 1, 0);
    expect(result.rectangle).toEqualEpsilon(
      expected.rectangle,
      CesiumMath.EPSILON9
    );
