...
defineSuite([
    ...
    it('A DynamicVector causes a primtive to be created and updated.', function() {
        ...
        expect(primitive.getPositions()).toEqual([testObject.position.getValue(time), Cartesian3.add(testObject.position.getValue(time), Cartesian3.normalize(vector.direction.getValue(time)).multiplyByScalar(vector.length.getValue(time)))]);
