...
test("Viewer renders network only", async (/** @type {{cesiumPage: CesiumPage}} */ {
  cesiumPage,
}) => {
  await cesiumPage.goto();

  await cesiumPage.page.evaluate(async () => {
    ...
    try {
      ...
      const terrainTileset =
        await Cesium.Cesium3DTileset.fromIonAssetId(2389063);
...
test("Viewer renders event based", async (/** @type {{cesiumPage: CesiumPage}} */ {
  cesiumPage,
}) => {
  await cesiumPage.goto();
  ...
  await cesiumPage.page.evaluate(async () => {
    ...
    try {
      ...
      await waitForAllPrimitives(viewer.scene, 20 * 1000);
    ...
    async function waitForAllPrimitives(scene, timeout = 1000 * 10) {
      const promises = [];
  ...
  // const viewerHandle = await cesiumPage.page.evaluateHandle(
  //   () => window.cesiumViewer,
  // );

  // viewerHandle.evaluate((viewer) => {
  //   console.log("prims", viewer.scene.primitives.length);
  // });

  await cesiumPage.page.waitForFunction("window.allTilesLoaded", {
    timeout: 60000,
  });
...
test("CesiumPage extension", async (/** @type {{cesiumPage: CesiumPage}} */ {
  cesiumPage,
}) => {
  await cesiumPage.goto();

  await cesiumPage.runTest(async (Cesium) => {
    const viewer = new Cesium.Viewer("cesiumContainer", {
      animation: false,
      timeline: false,
      geocoder: false,
    ...
    try {
      ...
      // TODO: this doesn't work right now, would need to find a way to pass the helper functions in
      // eslint-disable-next-line no-undef
      waitForAllPrimitives(viewer.scene, 20 * 1000).then(() => {
        console.log("all loaded");
        window.allTilesLoaded = true;
      });
    } catch (error) {
...
test("Full sandcastle", async ({ cesiumPage }) => {
  await cesiumPage.goto();
  ...
  await expect(cesiumPage.page).toHaveScreenshot({ update: true });
});
