...
defineSuite([
    ...
    it('show tile coords', function() {
        var viewModel = new CesiumInspectorViewModel(scene);
        expect(viewModel.scene.primitives.centralBody.imageryLayerCollection.length).toBe(0);

        viewModel.tileCoordinates  = true;
        viewModel.showTileCoordinates();
        expect(viewModel.scene.primitives.centralBody.imageryLayerCollection.length).toBe(1);
