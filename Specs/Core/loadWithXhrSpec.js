/*global defineSuite*/
defineSuite([
        'Core/loadWithXhr',
        'Core/loadImage',
        'Specs/waitsForPromise'
    ], function(
        loadWithXhr,
        loadImage,
        waitsForPromise) {
    "use strict";
    /*global jasmine,describe,xdescribe,it,xit,expect,beforeEach,afterEach,beforeAll,afterAll,spyOn,runs,waits,waitsFor*/

    describe('data URI loading', function() {
        it('can load URI escaped text with default response type', function(done) {
            loadWithXhr({
                url : 'data:,Hello%2C%20World!'
            }).then(function(result) {
                expect(result).toEqual('Hello, World!');
                done();
            });
        });

        it('can load URI escaped text with responseType=text', function(done) {
            loadWithXhr({
                url : 'data:,Hello%2C%20World!',
                responseType : 'text'
            }).then(function(result) {
                expect(result).toEqual('Hello, World!');
                done();
            });
        });

        it('can load Base64 encoded text with default response type', function(done) {
            loadWithXhr({
                url : 'data:text/plain;base64,SGVsbG8sIFdvcmxkIQ=='
            }).then(function(result) {
                expect(result).toEqual('Hello, World!');
                done();
            });
        });

        it ('can load Base64 encoded text with responseType=text', function(done) {
            loadWithXhr({
                url : 'data:text/plain;base64,SGVsbG8sIFdvcmxkIQ==',
                responseType : 'text'
            }).then(function(result) {
                expect(result).toEqual('Hello, World!');
                done();
            });
        });

        it('can load Base64 & URI encoded text with default responseType', function(done) {
            loadWithXhr({
                url : 'data:text/plain;base64,SGVsbG8sIFdvcmxkIQ%3D%3D'
            }).then(function(result) {
                expect(result).toEqual('Hello, World!');
                done();
            });
        });

        it('can load Base64 & URI encoded text with responseType=text', function(done) {
            loadWithXhr({
                url : 'data:text/plain;base64,SGVsbG8sIFdvcmxkIQ%3D%3D',
                responseType : 'text'
            }).then(function(result) {
                expect(result).toEqual('Hello, World!');
                done();
            });
        });

        it('can load URI escaped HTML as text with default responseType', function(done) {
            loadWithXhr({
                url : 'data:text/html,%3Ch1%3EHello%2C%20World!%3C%2Fh1%3E'
            }).then(function(result) {
                expect(result).toEqual('<h1>Hello, World!</h1>');
                done();
            });
        });

        it('can load URI escaped HTML as text with responseType=text', function(done) {
            loadWithXhr({
                url : 'data:text/html,%3Ch1%3EHello%2C%20World!%3C%2Fh1%3E',
                responseType : 'text'
            }).then(function(result) {
                expect(result).toEqual('<h1>Hello, World!</h1>');
                done();
            });
        });

        it('can load URI escaped text as JSON', function(done) {
            loadWithXhr({
                url : 'data:application/json,%7B%22key%22%3A%22value%22%7D',
                responseType : 'json'
            }).then(function(result) {
                expect(result.key).toEqual('value');
                done();
            });
        });

        it('can load Base64 encoded text as JSON', function(done) {
            loadWithXhr({
                url : 'data:application/json;base64,eyJrZXkiOiJ2YWx1ZSJ9',
                responseType : 'json'
            }).then(function(result) {
                expect(result.key).toEqual('value');
                done();
            });
        });

        it('can load URI escaped HTML as document', function(done) {
            loadWithXhr({
                url : 'data:text/html,%3Ch1%3EHello%2C%20World!%3C%2Fh1%3E',
                responseType : 'document'
            }).then(function(result) {
                expect(result.querySelector('h1')).not.toBeNull();
                done();
            });
        });

        var tile = 'data:;base64,';

        it('can load Base64 encoded data as arraybuffer', function(done) {
            loadWithXhr({
                url : tile,
                responseType : 'arraybuffer'
            }).then(function(result) {
                expect(result.byteLength).toEqual(3914);
                done();
            });
        });

        var image = 'data:image/png;base64,';

        it('can load Base64 encoded data as blob', function(done) {
            loadWithXhr({
                url : image,
                responseType : 'blob'
            }).then(function(result) {
                expect(result.type).toEqual('image/png');

                /*global URL*/
                var blobUrl = URL.createObjectURL(result);

                return loadImage(blobUrl).then(function(image) {
                    expect(image.width).toEqual(24);
                    expect(image.height).toEqual(24);

                    done();
                });
            });
        });
    });
});