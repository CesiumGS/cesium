...
describe("Core/ProjectedImageryTilingScheme", function () {
  ...
  describe("utilities for getting information about projected tiles.", function () {
    it("gets the projected rectangle for a projected tile", function () {
      ...
      expect(rectangle.west).toEqualEpsilon(
        mollweideProjectedRectangle.west,
        CesiumMath.EPSILON10
      );
      expect(rectangle.south).toEqualEpsilon(
        mollweideProjectedCenter.latitude,
        CesiumMath.EPSILON10
      );
      expect(rectangle.east).toEqualEpsilon(
        mollweideProjectedCenter.longitude,
        CesiumMath.EPSILON10
      );
      expect(rectangle.north).toEqualEpsilon(
        mollweideProjectedRectangle.north,
        CesiumMath.EPSILON10
      );
      ...
      expect(rectangle.south).toEqualEpsilon(
        mollweideProjectedRectangle.south,
        CesiumMath.EPSILON10
      );
      expect(rectangle.east).toEqualEpsilon(
        mollweideProjectedRectangle.east,
        CesiumMath.EPSILON10
      );
      expect(rectangle.north).toEqualEpsilon(
        mollweideProjectedCenter.latitude,
