defineSuite(['Core/JulianDate', 'Core/TimeStandard', 'Core/TimeConstants', 'Core/Math'], function(JulianDate, TimeStandard, TimeConstants, CesiumMath) {
    ...
    it("Construct a default date", function() {
        ...
        var dateNow = JulianDate.createFromDate(new Date());
    ...
    it("Construct a date from a JavaScript Date (4).", function() {
        ...
        var julianDate = JulianDate.createFromDate(jsDate);
    ...
    it("Construct a date from a JavaScript Date in different TimeStandard.", function() {
        ...
        var taiJulianDate = JulianDate.createFromDate(taiDate, TimeStandard.TAI);

        var utcDate = new Date("September 1, 2011 11:59:26");
        var utcJulianDate = JulianDate.createFromDate(utcDate, TimeStandard.UTC);
    ...
    it("Fail to construct from an undefined JavaScript Date.", function() {
        expect(function() {
            return JulianDate.createFromDate(undefined);
        }).toThrow();
    });

    it("Fail to construct from a null JavaScript Date.", function() {
        expect(function() {
            return JulianDate.createFromDate(null);
        }).toThrow();
    });

    it("Fail to construct from an invalid JavaScript Date.", function() {
        expect(function() {
            return JulianDate.createFromDate(new Date(Date.parse("garbage")));
        }).toThrow();
    });

    it("Fail to construct from a non-date JavaScript Date.", function() {
        expect(function() {
            return JulianDate.createFromDate(0);
        }).toThrow();
    });

    it("Fail to construct from a JavaScript Date with null time standard.", function() {
        expect(function() {
            return JulianDate.createFromDate(new Date(), null);
        }).toThrow();
    });

    it("Fail to construct from a JavaScript Date with invalid time standard.", function() {
        expect(function() {
            return JulianDate.createFromDate(new Date(), "invalid");
        }).toThrow();
    });

    it("Construct a date from total days (1).", function() {
        var julianDate = JulianDate.createFromTotalDays(2448257.75, TimeStandard.UTC);
        expect(julianDate.equals(JulianDate.createFromDate(new Date("January 1, 1991 06:00:00 UTC")))).toBeTruthy();
    });

    it("Construct a date from total days (2).", function() {
        var julianDate = JulianDate.createFromTotalDays(2455747.0, TimeStandard.UTC);
        expect(julianDate.equals(JulianDate.createFromDate(new Date("July 4, 2011 12:00:00 UTC")))).toBeTruthy();
    });

    it("Construct a date from total days (3).", function() {
        var julianDate = JulianDate.createFromTotalDays(2459580.25, TimeStandard.UTC);
        expect(julianDate.equals(JulianDate.createFromDate(new Date("December 31, 2021 18:00:00 UTC")))).toBeTruthy();
    });

    it("Construct a date from total days with different time standards.", function() {
        var julianDate = JulianDate.createFromTotalDays(2455806, TimeStandard.TAI);
        expect(julianDate.equals(JulianDate.createFromDate(new Date("September 1, 2011 12:00:00 UTC"), TimeStandard.TAI))).toBeTruthy();
    });

    it("Fail to construct from non-numeric total days.", function() {
        expect(function() {
            return JulianDate.createFromTotalDays("not a number", TimeStandard.UTC);
        }).toThrow();
    });

    it("Fail to construct from null total days.", function() {
        expect(function() {
            return JulianDate.createFromTotalDays(null, TimeStandard.UTC);
        }).toThrow();
    });

    it("Fail to construct from undefined total days.", function() {
        expect(function() {
            return JulianDate.createFromTotalDays(undefined, TimeStandard.UTC);
        }).toThrow();
    });

    it("Fail to construct from total days with null time standard.", function() {
        expect(function() {
            return JulianDate.createFromTotalDays(1234, null);
        }).toThrow();
    });

    it("Fail to construct from total days with invalid time standard.", function() {
        expect(function() {
            return JulianDate.createFromTotalDays(1234, "invalid");
        }).toThrow();
    });
    ...
    it("getDate works when constructed from total days", function() {
        var julianDate = JulianDate.createFromTotalDays(2455770.9986087964, TimeStandard.UTC);
    ...
    it("getDate works when using TAI", function() {
        var julianDate = JulianDate.createFromTotalDays(2455927.157772, TimeStandard.UTC);
    ...
    it("getSecondsDifference works in UTC", function() {
        var start = JulianDate.createFromDate(new Date("July 4, 2011 12:00:00 UTC"));
        var end = JulianDate.createFromDate(new Date("July 5, 2011 12:01:00 UTC"));
    ...
    it("addSeconds works with whole seconds", function() {
        var start = JulianDate.createFromDate(new Date("July 4, 2011 12:00:30 UTC"));
    ...
    it("addSeconds works with fractions (1)", function() {
        var start = JulianDate.createFromDate(new Date("July 4, 2011 23:59:59 UTC"));
    ...
    it("addSeconds works with fractions (2)", function() {
        var start = JulianDate.createFromDate(new Date("August 11 2011 6:00:00 UTC"));
    ...
    it("addSeconds works with fractions (3)", function() {
        var start = JulianDate.createFromDate(new Date("August 11 2011 11:59:59 UTC"));
    ...
    it("addSeconds works with negative numbers", function() {
        var start = JulianDate.createFromDate(new Date("July 4, 2011 12:01:30 UTC"));
    ...
    it("toYearFraction works (1)", function() {
        var julianDate = JulianDate.createFromDate(new Date("January 2, 2011 0:00:00 UTC"));
    ...
    it("toYearFraction works (2)", function() {
        var julianDate = JulianDate.createFromDate(new Date("January 1, 2011 18:00:00 UTC"));
        var yearFraction = julianDate.toYearFraction();
        expect(yearFraction).toEqualEpsilon(0.75 / 365.0, CesiumMath.EPSILON10);
    });

    it("toYearFraction works in leap years", function() {
        var julianDate = JulianDate.createFromDate(new Date("January 2, 2012 0:00:00 UTC"));
        var yearFraction = julianDate.toYearFraction();
    ...
    it("lessThan works with equal values", function() {
        var start = JulianDate.createFromDate(new Date("July 6, 1991 12:00:00"));
        var end = JulianDate.createFromDate(new Date("July 6, 1991 12:00:00"));
    ...
    it("lessThan works with different time standards", function() {
        var start = JulianDate.createFromDate(new Date("July 6, 1991 12:00:00"), TimeStandard.UTC);
        var end = JulianDate.createFromDate(new Date("July 6, 2011 12:00:00"), TimeStandard.TAI);
        expect(start.lessThan(end)).toBeTruthy();
    });
    ...
    it("greaterThan works", function() {
        var start = JulianDate.createFromDate(new Date("July 6, 2011 12:01:00"));
        var end = JulianDate.createFromDate(new Date("July 6, 1991 12:00:00"));
    ...
    it("greaterThan works with different time standards", function() {
        var start = JulianDate.createFromDate(new Date("July 6, 2011 12:01:00"), TimeStandard.TAI);
        var end = JulianDate.createFromDate(new Date("July 6, 1991 12:00:00"), TimeStandard.UTC);
        expect(start.greaterThan(end)).toBeTruthy();
    });
    ...
    it("can be equal to within an epsilon of another JulianDate", function() {
        var original = JulianDate.createFromDate(new Date("September 7, 2011 12:55:00 UTC"));
        var clone = JulianDate.createFromDate(new Date("September 7, 2011 12:55:00 UTC"));
    ...
    it("getTotalDays works", function() {
        ...
        var original = JulianDate.createFromTotalDays(totalDays);
        expect(totalDays).toEqual(original.getTotalDays());
    });

    it("equalsEpsilon works", function() {
        var original = new JulianDate();
        var clone = JulianDate.createFromDate(original.getDate());
    ...
    it("getTaiMinusUtc works between TAI and UTC (1)", function() {
        ...
        var jd = JulianDate.createFromDate(date, TimeStandard.TAI);
